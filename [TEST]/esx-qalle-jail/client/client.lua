...
RegisterNetEvent("esx-qalle-jail:openJail1Menu")
AddEventHandler("esx-qalle-jail:openJail1Menu", function()
	OpenJailMenu()
end)

RegisterNetEvent("esx-qalle-jail:jail1Player")
AddEventHandler("esx-qalle-jail:jail1Player", function(newJailTime)
	jailTime = newJailTime

	Cutscene()
end)

RegisterNetEvent("esx-qalle-jail:unJail1Player")
AddEventHandler("esx-qalle-jail:unJail1Player", function()
	jailTime = 0
...
function JailLogin()
	...
	ESX.ShowNotification("La dernière fois que vous vous êtes endormi, vous avez été emprisonné, à cause de cela, vous êtes maintenant remis !")
...
function UnJail()
	...
	ESX.ShowNotification("Vous êtes libéré, restez calme à l'extérieur ! Bonne chance !")
end
...
function InJail()
	...
	Citizen.CreateThread(function()
		...
		while jailTime > 0 do
			...
			ESX.ShowNotification("Il vous reste " .. jailTime .. " minutes de prison !")
					...
					if DistanceCheck <= 1.5 then
						...
						if IsControlJustPressed(0, 38) then
							...
							if v["state"] then
								...
								ESX.ShowNotification("Vous avez déjà prit ce paquet !")
							end
...
function DeliverPackage(packageId)
	...
	while Packaging do
		...
		if not IsEntityAttachedToEntity(packageId, PlayerPedId()) then
			...
			ESX.Game.Utils.DrawText3D(DeliverPosition, "[E] Poser Package", 0.4)
...
function OpenJailMenu()
	ESX.UI.Menu.Open(
		'default', GetCurrentResourceName(), 'jail_prison_menu',
		{
			...
			align    = 'center',
			elements = {
				{ label = "Emprisonner un joueur", value = "jail_closest_player" },
				{ label = "Libérer un joueur", value = "unjail_player" }
			}
		}, 
	function(data, menu)
			...
			ESX.UI.Menu.Open(
          	...
          	function(data2, menu2)
            	...
            	if jailTime == nil then
              		ESX.ShowNotification("Le temps doit être de quelques minutes !")
            	else
              		menu2.close()
						...
						function(data3, menu3)
						  	...
						  	if reason == nil then
								...
								if closestPlayer == -1 or closestDistance > 3.0 then
								  	...
								  	TriggerServerEvent("esx-qalle-jail:jail1Player", GetPlayerServerId(closestPlayer), jailTime, reason)
								end
		...
		elseif action == "unjail_player" then
			...
			ESX.TriggerServerCallback("esx-qalle-jail:retrieveJailedPlayers", function(playerArray)
				...
				for i = 1, #playerArray, 1 do
					table.insert(elements, {label = "Prisonnier : " .. playerArray[i].name .. " | Temps Restant : " .. playerArray[i].jailTime .. " minutes", value = playerArray[i].identifier })
				end

				ESX.UI.Menu.Open(
					'default', GetCurrentResourceName(), 'jail_unjail_menu',
					{
						title = "Libérer un joueur",
						align = "center",
						elements = elements
					},
				function(data2, menu2)
					...
					TriggerServerEvent("esx-qalle-jail:unJail1Player", action)
