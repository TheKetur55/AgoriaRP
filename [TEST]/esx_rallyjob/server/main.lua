...
AddEventHandler('esx_rallyjob:onRecord', function(track, laptime)						
	...
	if not found then
		...
		TriggerClientEvent('esx:showNotification', _source, '~g~Nouveau ~w~record de piste!~g~+~w~1500~g~€ ~w~bonus!')
	...
	elseif found and record then
		table.insert(RallyTimes, {id = -1 , track = _track, driver = _driver, laptime = _laptime})
		xPlayer.addMoney(1500)
		TriggerClientEvent('esx:showNotification', _source, '~g~Vous avez battu~w~ le record de piste! ~g~+~w~1500~g~$ ~w~bonus!')
		...
		TriggerClientEvent('esx:showNotification', _source, '~w~Malheureusement votre record ~r~a été battu sur le chemin du retour!')
	end
end)
