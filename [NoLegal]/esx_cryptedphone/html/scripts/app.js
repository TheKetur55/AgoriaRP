(function(){
	...
	let showGPS = function(xPos, yPos) {
		$.post('http://esx_cryptedphone/setGPS', JSON.stringify({
			x: parseFloat(xPos),
			y: parseFloat(yPos)
		}
		));
	...
	let addMessage = function(phoneNumber, pmessage, pposition, panonyme, pjob){
		...
		$('.message .ok-btn').click(function() {
			$.post('http://esx_cryptedphone/send', JSON.stringify({
	...
	$('#contact_send').click(function(){
		...
		$.post('http://esx_cryptedphone/add_contact', JSON.stringify({
	...
	window.onData = function(data){
		...
		if(data.enterPressed){
			...
			if(isPhoneShowed) {
				...
				$.post('http://esx_cryptedphone/select', JSON.stringify({
					val  : currentVal
				}));
	...
	document.onkeydown = function (data) {
		if ((data.which == 120 || data.which == 27) && isPhoneShowed) { // || data.which == 8
			$.post('http://esx_cryptedphone/escape');
		}
	};
