...
AddEventHandler('sellDrugs', function()
	...
	TriggerClientEvent('esx:showNotification', _source, _U('you_have_sold') .. '~b~'..x..'~w~' .. 'Weed' .. blackMoney .. '$')
	
	if Config.EnableJobLogs == true then
					TriggerEvent('esx_joblogs:AddInLog', 'venteweed', 'venteweed_action', xPlayer.name,x, blackMoney)
				end
end)
...
AddEventHandler('drugsInProgress', function(street1, street2, sex)
	...
	TriggerClientEvent("outlawNotify", -1, "~r~ "..sex.."~w~ vend de la drogue ~r~entre ~w~"..street1.."~r~ et ~w~"..street2)
end)
...
AddEventHandler('drugsInProgressS1', function(street1, sex)
	...
	TriggerClientEvent("outlawNotify", -1, "~r~ "..sex.."~w~ vend de la drogue dans la rue ~w~"..street1)
end)
