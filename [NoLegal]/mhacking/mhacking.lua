	...
	while true do
		...
		if showHelp then
			if helpTimer > GetGameTimer() then
				showHelpText("Naviguer avec ~y~W,A,S,D~s~ et confirmez avec ~y~SPACE~s~ pour le bloc de code a gauche.")
			elseif helpTimer > GetGameTimer()-helpCycle then
				showHelpText("Utilisez les ~y~Fléche Droiit/Gauche/Haut/Bas~s~ et ~y~ENTER~s~ pour le bloc de code a droite")
			else
