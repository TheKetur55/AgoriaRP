...
RegisterNetEvent("esx_blanchisseur2:notify")
AddEventHandler("esx_blanchisseur2:notify", function(icon, type, sender, title, text)
    Citizen.CreateThread(function()
		Wait(1)
		...
		if data.current.value == 'wash_money' then
			...
			ESX.UI.Menu.Open(
				...
				function(data, menu)
					...
					if amount == nil then
						TriggerEvent("esx_blanchisseur2:notify", "CHAR_LESTER_DEATHWISH", 1, _U('Notification'), false, _U('invalid_amount'))
						ESX.UI.Menu.CloseAll()
					else
						menu.close()
						TriggerServerEvent('esx_blanchisseur2:washMoney', amount)
						ESX.UI.Menu.CloseAll()
					end
...
AddEventHandler('esx_blanchisseur2:hasEnteredMarker', function(zone)
	CurrentAction     = 'whitening'
	CurrentActionMsg  = _U('press_menu')
	CurrentActionData = {zone = zone}
end)

AddEventHandler('esx_blanchisseur2:hasExitedMarker', function(zone)
	CurrentAction = nil
	TriggerServerEvent('esx_blanchisseur2:stopWhitening')
end)
...
Citizen.CreateThread(function()
	while true do
		...
		if isInMarker and not HasAlreadyEnteredMarker then
			...
			TriggerEvent('esx_blanchisseur2:hasEnteredMarker', currentZone)
		end
		if not isInMarker and HasAlreadyEnteredMarker then
			HasAlreadyEnteredMarker = false
			TriggerEvent('esx_blanchisseur2:hasExitedMarker', LastZone)
		end
	end
end)
			...
			if Config.Hours then
				if heure > Config.openHours and heure < Config.closeHours then
					if Config.Menu then	
						...
						TriggerServerEvent('esx_blanchisseur2:startWhitening', percent)
					end					
				else
					TriggerServerEvent('esx_blanchisseur2:Nothere')
				end
			else
				if Config.Menu then	
					OpenBlanchisseurMenu()
