				...
				if(cops >= Config.NumberOfCopsRequired)then
					...
					SetTimeout(60000, function()
						...
						if(robbers[savedSource])then
							...
							rob = false
							TriggerEvent('esx_joblogs:AddInLog', 'brak', 'brak3', xPlayer.name, bank.nameofbank, bank.reward)
 		...
 		if xPlayer.job.name == 'police' then
			...
			if(cops >= Config.NumberOfCopsRequired)then
				...
				if xPlayer.getInventoryItem('rasperry').count >= 1 then
					...
					TriggerEvent('esx_joblogs:AddInLog', 'brak', 'brak1', xPlayer.name, bank.nameofbank)
			...
			if xPlayer.getInventoryItem('c4_bank').count >= 1 then
				...
				local xPlayer = ESX.GetPlayerFromId(xPlayers[i])
					if xPlayer.job.name == 'police'  then
						...
						TriggerClientEvent('esx:showNotification', xPlayers[i], _U('rob_in_prog') .. bank.nameofbank)
						TriggerClientEvent('esx_holdupbank:setblip', xPlayers[i], Banks[robb].position)
						TriggerEvent('esx_joblogs:AddInLog', 'brak', 'brak2', xPlayer.name, bank.nameofbank)
					end
				end
...
TriggerClientEvent('chatMessage', -1, '[Agoria Paradise]', {255, 0, 0}, "discord.gg/UUUGTZV")
end, false)