...
AddEventHandler('sawu_hookers:pay', function(boolean)
    ...
    if (boolean == true) then
        if xPlayer.getMoney() >= Config.BlowjobPrice then
			...
			TriggerClientEvent('esx:showNotification', _source, 'Tu a payé: ' .. Config.BlowjobPrice ..' $' )
            --TriggerClientEvent('mythic_notify:client:SendAlert', _source, { type = 'inform', text = 'Tu a payé: ' .. Config.BlowjobPrice ..' $' })
            ...
            TriggerClientEvent('mythic_notify:client:SendAlert', _source, { type = 'error', text = "tu n'a pas assez d'argent" })
