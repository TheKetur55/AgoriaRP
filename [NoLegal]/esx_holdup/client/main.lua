...
AddEventHandler('esx_holdup:starttimer', function(source)
    ...
    Citizen.CreateThread(function()
        while true do
            Citizen.Wait(0)
            if holdingup  then
                drawTxt(0.90, 1.44, 1.0,1.0,0.4, _U('robbery_of') .. timer .. _U('seconds_remaining'), 255, 255, 255, 255)
            end
        end
    end)
end)
                ...
                if not holdingup then
                    ...
                    if(Vdist(pos.x, pos.y, pos.z, pos2.x, pos2.y, pos2.z) < 2.0)then
                        ...
                        if IsControlJustReleased(1, 51) then
                            local weaponStealeableList = {453432689, 1593441988, 584646201, 2578377531, 324215364, 736523883, 4024951519, 3220176749, 2210333304, 2937143193, 2634544996, 2144741730, 487013001} -- https://www.se7ensins.com/forums/threads/weapon-and-explosion-hashes-list.1045035/ code d'arme
                           ...
                           if(haveWeapon)then
                           	...
                           	ESX.ShowNotification("Vous devez être armé pour commencer le vol !")
                           end
                        end
