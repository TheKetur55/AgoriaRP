	...
	ESX.TriggerServerCallback('esx_carthief:isActive', function(isActive)
		if isActive == 0 then
			ESX.TriggerServerCallback('esx_carthief:anycops', function(anycops)
				...
				--	TriggerEvent('esx_joblogs:AddInLog', 'drogue', 'cokev', GetPlayerName(PlayerId() ))
					...
					ESX.ShowNotification("Pas assez de flics en ville!")
				end
			end)
		else
			ESX.ShowNotification("Il y a déjà un vol de voiture en cours!")
		end
	end)
end
...
AddEventHandler('esx_carthief:setcopnotification', function()
	ESX.ShowNotification("Vol de voiture en cour. Le suivi du véhicule sera actif sur votre radar")
end)

AddEventHandler('esx_carthief:hasEnteredMarker', function(zone)
  if LastZone == 'menucarthief' then
    CurrentAction     = 'carthief_menu'
    CurrentActionMsg  = 'Appuyer sur ~INPUT_CONTEXT~ pour voler une voiture'
    CurrentActionData = {zone = zone}
  elseif LastZone == 'cardelivered' then
    CurrentAction     = 'cardelivered_menu'
    CurrentActionMsg  = 'Appuyer sur ~INPUT_CONTEXT~ pour déposer la voiture'
    CurrentActionData = {zone = zone}
  end
end)
