...
ESX.SavePlayer = function(xPlayer, cb)
	...
	table.insert(asyncTasks, function(cb)
		MySQL.Async.execute('UPDATE users SET `job` = @job, `job_grade` = @job_grade, `loadout` = @loadout, `second_job` = @second_job, `second_job_grade` = @second_job_grade, `position` = @position WHERE identifier = @identifier', {
			['@job']        = xPlayer.job.name,
			['@job_grade']  = xPlayer.job.grade,
			['@loadout']    = json.encode(xPlayer.getLoadout()),
			['@second_job'] = xPlayer.second_job.name,
			['@second_job_grade'] = xPlayer.second_job.grade,
