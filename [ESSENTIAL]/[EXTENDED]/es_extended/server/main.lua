AddEventHandler('es:playerLoaded', function(source, _player)
	...
	TriggerEvent('es:getPlayerFromId', _source, function(player)
		...
		-- Get second_job and loadout
		table.insert(tasks, function(cb)

			local tasks2 = {}

			-- Get second_job name, grade and last position
			table.insert(tasks2, function(cb2)
				...
				MySQL.Async.fetchAll(
					'SELECT * FROM `users` WHERE `identifier` = @identifier', {
						...
						userData.second_job['name']  = result[1].second_job
						userData.second_job['grade'] = result[1].second_job_grade
      ...
      -- Get second_job label
			table.insert(tasks2, function(cb2)
				MySQL.Async.fetchAll('SELECT * FROM `jobs` WHERE `name` = @name',{
					['@name'] = userData.second_job.name
				}, function(result)
					userData.second_job['label'] = result[1].label

					cb2()
				end)
			end)

      -- Get second_job grade data
			table.insert(tasks2, function(cb2)
				MySQL.Async.fetchAll('SELECT * FROM `job_grades` WHERE `job_name` = @job_name AND `grade` = @grade',{
					['@job_name'] = userData.second_job.name,
					['@grade']    = userData.second_job.grade
				},function(result)

					userData.second_job['grade_name']   = result[1].name
					userData.second_job['grade_label']  = result[1].label
					userData.second_job['grade_salary'] = result[1].salary

					userData.second_job['skin_male']   = {}
					userData.second_job['skin_female'] = {}

					if result[1].skin_male ~= nil then
						userData.second_job['skin_male'] = json.decode(result[1].skin_male)
					end

					if result[1].skin_female ~= nil then
						userData.second_job['skin_female'] = json.decode(result[1].skin_female)
					end
		...
		Async.parallel(tasks, function(results)
			...
			local xPlayer = CreateExtendedPlayer(player, userData.accounts, userData.inventory, userData.job, userData.loadout, userData.second_job, userData.playerName, userData.lastPosition)
			...
			xPlayer.getMissingAccounts(function(missingAccounts)
				...
				TriggerClientEvent('esx:playerLoaded', _source, {
					...
					second_job   = xPlayer.getSecondJob(),
...
AddEventHandler('esx:giveInventoryItem', function(target, type, itemName, itemCount)
	...
	if type == 'item_standard' then
		...
		if itemCount > 0 and sourceItem.count >= itemCount then
			...
			if targetItem.limit ~= -1 and (targetItem.count + itemCount) > targetItem.limit then
				TriggerClientEvent('esx:showNotification', _source, _U('ex_inv_lim'))
			else
				sourceXPlayer.removeInventoryItem(itemName, itemCount)
				targetXPlayer.addInventoryItem   (itemName, itemCount)
				
				TriggerClientEvent('esx:showNotification', _source, _U('gave_item', itemCount, ESX.Items[itemName].label))
				TriggerEvent('esx_joblogs:AddInLog', 'donneritem', 'donneritem', sourceXPlayer.name, itemCount,ESX.Items[itemName].label, targetXPlayer.name)
				TriggerClientEvent('esx:showNotification', target,  _U('received_item', itemCount, ESX.Items[itemName].label))
			end
	...
	elseif type == 'item_money' then
		...
		if itemCount > 0 and sourceXPlayer.getMoney() >= itemCount then
			...
			TriggerEvent('esx_joblogs:AddInLog', 'donnerargent', 'donnerargent', sourceXPlayer.name, itemCount, targetXPlayer.name)
	...
	elseif type == 'item_weapon' then
		...
		if not targetXPlayer.hasWeapon(itemName) then
			...
			if itemCount > 0 then
				TriggerClientEvent('esx:showNotification', _source, _U('gave_weapon_ammo', weaponLabel, itemCount))
				TriggerEvent('esx_joblogs:AddInLog', 'donnerarme1', 'donneritem', sourceXPlayer.name, itemCount, weaponLabel, targetXPlayer.name)
				TriggerClientEvent('esx:showNotification', target,  _U('received_weapon_ammo', weaponLabel, itemCount))
			else
				TriggerClientEvent('esx:showNotification', _source, _U('gave_weapon', weaponLabel))
				TriggerEvent('esx_joblogs:AddInLog', 'donnerarme', 'donneritem', sourceXPlayer.name, weaponLabel, targetXPlayer.name)
				TriggerClientEvent('esx:showNotification', target,  _U('received_weapon', weaponLabel))
			end

		else
			TriggerClientEvent('esx:showNotification', _source, _U('gave_weapon_hasalready', weaponLabel))
			TriggerClientEvent('esx:showNotification', _source, _U('received_weapon_hasalready', weaponLabel))
		end
	...
	if type == 'item_standard' then
		...
		if itemCount == nil or itemCount < 1 then
			...
			if (itemCount > xItem.count or xItem.count < 1) then
				...
				TriggerEvent('esx_joblogs:AddInLog', 'jeter', 'jeter', player.name, xItem.label,itemCount)
				--HELP
