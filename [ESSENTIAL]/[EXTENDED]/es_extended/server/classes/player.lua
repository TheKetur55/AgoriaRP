function CreateExtendedPlayer(player, accounts, inventory, job, loadout, second_job, name, lastPosition)
	local self = {}
	...
	self.setSecondJob = function(name, grade)
		local lastJob = json.decode(json.encode(self.second_job))

		MySQL.Async.fetchAll('SELECT * FROM `jobs` WHERE `name` = @name',{
			['@name'] = name
		},function(result)

			self.second_job['id']    = result[1].id
			self.second_job['name']  = result[1].name
			self.second_job['label'] = result[1].label

			MySQL.Async.fetchAll('SELECT * FROM `job_grades` WHERE `job_name` = @job_name AND `grade` = @grade',{
				['@job_name'] = name,
				['@grade']    = grade
			},function(result)
				self.second_job['grade']        = grade
				self.second_job['grade_name']   = result[1].name
				self.second_job['grade_label']  = result[1].label
				self.second_job['grade_salary'] = result[1].salary

				self.second_job['skin_male']    = nil
				self.second_job['skin_female']  = nil

				if result[1].skin_male ~= nil then
					self.second_job['skin_male'] = json.decode(result[1].skin_male)
				end

				if result[1].skin_female ~= nil then
					self.second_job['skin_female'] = json.decode(result[1].skin_female)
				end

				TriggerEvent("esx:setSecondJob", self.source, self.second_job, lastJob)
				TriggerClientEvent("esx:setSecondJob", self.source, self.second_job)
