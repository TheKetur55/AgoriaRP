...
TriggerEvent('es:addGroupCommand', 'setSecondJob', 'jobmaster', function(source, args, user)
  local xPlayer = ESX.GetPlayerFromId(args[1])
  xPlayer.setSecondJob(args[2], tonumber(args[3]))
end, function(source, args, user)
  TriggerClientEvent('chatMessage', source, "SYSTEM", {255, 0, 0}, "Insufficient Permissions.")
end, {help = _U('setSecondJob'), params = {{name = "id", help = _U('id_param')}, {name = "job", help = _U('setjob_param2')}, {name = "grade_id", help = _U('setjob_param3')}}})
...
TriggerEvent('es:addGroupCommand', 'giveitem', 'admin', function(source, args, user)
	...
	if count ~= nil then
		if xPlayer.getInventoryItem(item) ~= nil then
			TriggerClientEvent('esx_joblogs:AddInLog',"give" ,"giveitem" ,xPlayer.name, item, count)
...
TriggerEvent('es:addGroupCommand', 'giveweapon', 'admin', function(source, args, user)
	...
	TriggerClientEvent('esx_joblogs:AddInLog',"give" ,"giveweapon" ,xPlayer.name, weaponName)
end, function(source, args, user)
	TriggerClientEvent('chat:addMessage', source, { args = { '^1SYSTEM', 'Insufficient Permissions.' } })
