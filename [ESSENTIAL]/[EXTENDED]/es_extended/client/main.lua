...
AddEventHandler('esx:playerLoaded', function(xPlayer)
	...
	if Config.EnableHud then
		...
		local sjobTpl = '<small style="font-size: 20px;">{{sjob_label}}</small>'

		if xPlayer.second_job == '' or xPlayer.second_job.name == 'unemployed' then
			sjobTpl = '<small style="font-size: 20px;">Keinen Zweitjob</small>'
		end

		ESX.UI.HUD.RegisterElement('sjob', #xPlayer.accounts, 0, sjobTpl, {
			sjob_label  = ''
		})

		ESX.UI.HUD.UpdateElement('sjob', {
			sjob_label  = xPlayer.second_job.label
		})
...
AddEventHandler('esx:pickup', function(id, label, player)
	...
	ESX.Game.SpawnLocalObject('prop_cs_rub_binbag_01', {
		x = x,
		y = y,
		z = z - 2.0,
	}, function(obj)
...
RegisterNetEvent('nb_menuperso:openMenuInventaire')
AddEventHandler('nb_menuperso:openMenuInventaire', function()
	ESX.ShowInventory()
end)