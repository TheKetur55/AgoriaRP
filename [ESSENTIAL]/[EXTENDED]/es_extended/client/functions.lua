...
for i=1, #ESX.PlayerData.accounts, 1 do
		if ESX.PlayerData.accounts[i].name == 'black_money' then
			...
			--local canDrop = ESX.PlayerData.accounts[i].name ~= 'black_money'
			...
			if data1.current.action == 'give' then
				...
				for i=1, #players, 1 do
					if players[i] ~= PlayerId() then
						...
						table.insert(elements, {
					--		label = 'ID [' .. GetPlayerServerId(players[i]) .. '] Nom :' .. GetPlayerName(players[i]),
					label = 'ID [' .. GetPlayerServerId(players[i]) .. '] ',
					--	label =GetPlayerServerId(PlayerId())  GetPlayerName(players[i]),
