...
Citizen.CreateThread(function ()
	...
	ESX.TriggerServerCallback('ktr_truckshop:getCategories', function (categories)
		Categories = categories
	end)

	ESX.TriggerServerCallback('ktr_truckshop:getVehicles', function (vehicles)
		Vehicles = vehicles
	end)
...
RegisterNetEvent('ktr_truckshop:sendCategories')
AddEventHandler('ktr_truckshop:sendCategories', function (categories)
	Categories = categories
end)

RegisterNetEvent('ktr_truckshop:sendVehicles')
AddEventHandler('ktr_truckshop:sendVehicles', function (vehicles)
	Vehicles = vehicles
end)
...
function ReturnVehicleProvider()
	ESX.TriggerServerCallback('ktr_truckshop:getCommercialVehicles', function (vehicles)
		local elements = {}
		local returnPrice
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'return_provider_menu', {
			...
			TriggerServerEvent('ktr_truckshop:returnProvider', data.current.value)
...
function OpenShopMenu()
	...
	for i=1, #Vehicles, 1 do
		if IsModelInCdimage(GetHashKey(Vehicles[i].model)) then
			...
			print(('ktr_truckshop: vehicle "%s" does not exist'):format(Vehicles[i].model))
		end
	end
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_shop', {
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'shop_confirm', {
			...
			if data2.current.value == 'yes' then
				if Config.EnablePlayerManagement then
					ESX.TriggerServerCallback('ktr_truckshop:buyVehicleSociety', function(hasEnoughMoney)
					...
					if Config.EnableSocietyOwnedVehicles and playerData.job.grade_name == 'boss' then
						ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'shop_confirm_buy_type', {
							...
							if data3.current.value == 'personnal' then
								...
								ESX.TriggerServerCallback('ktr_truckshop:buyVehicle', function(hasEnoughMoney)
									if hasEnoughMoney then
										IsInShopMenu = false
										...
										ESX.Game.SpawnVehicle(vehicleData.model, Config.Zones.ShopOutside.Pos, Config.Zones.ShopOutside.Heading, function (vehicle)
											...
											if Config.EnableOwnedVehicles then
												TriggerServerEvent('ktr_truckshop:setVehicleOwned', vehicleProps)
								...
								ESX.TriggerServerCallback('ktr_truckshop:buyVehicleSociety', function (hasEnoughMoney)
									if hasEnoughMoney then
										...
										ESX.Game.SpawnVehicle(vehicleData.model, Config.Zones.ShopOutside.Pos, Config.Zones.ShopOutside.Heading, function (vehicle)
											...
											vehicleProps.plate = newPlate
											SetVehicleNumberPlateText(vehicle, newPlate)
											TriggerServerEvent('ktr_truckshop:setVehicleOwnedSociety', playerData.job.name, vehicleProps)
											if Config.EnableJobLogs == true then
													TriggerServerEvent('esx_joblogs:AddInLog', 'vehicleshop', 'selfcarbuy', GetPlayerName(PlayerId()), vehicleProps.plate, vehicleData.model)
												end
											ESX.ShowNotification(_U('vehicle_purchased'))
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'reseller', {
		...
		elseif action == 'set_vehicle_owner_sell' then
			...
			if closestPlayer == -1 or closestDistance > 3.0 then
				...
				TriggerServerEvent('ktr_truckshop:sellVehicle', model)
				TriggerServerEvent('ktr_truckshop:addToList', GetPlayerServerId(closestPlayer), model, newPlate)

				if Config.EnableOwnedVehicles then
					TriggerServerEvent('ktr_truckshop:setVehicleOwnedPlayerId', GetPlayerServerId(closestPlayer), vehicleProps)
					

--	if Config.EnableJobLogs == true then
	--TriggerServerEvent('esx_joblogs:AddInLog', 'vehicleshop', 'carbuy', GetPlayerName(closestPlayer), GetPlayerName(PlayerId()), plate, model)
	--end
			...
			if closestPlayer == -1 or closestDistance > 3.0 then
				...
				ESX.TriggerServerCallback('esx:getOtherPlayerData', function (xPlayer)
					...
					if Config.EnableSocietyOwnedVehicles then
						TriggerServerEvent('ktr_truckshop:setVehicleOwnedSociety', xPlayer.job.name, vehicleProps)
				--		if Config.EnableJobLogs == true then
				--			TriggerServerEvent('esx_joblogs:AddInLog', 'vehicleshop', 'carbuysociety', GetPlayerName(closestPlayer), GetPlayerName(PlayerId()), plate, model)
					--	end
						ESX.ShowNotification(_U('vehicle_set_owned', vehicleProps.plate, GetPlayerName(closestPlayer)))
					else
						ESX.ShowNotification(_U('vehicle_sold_to', vehicleProps.plate, GetPlayerName(closestPlayer)))
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'set_vehicle_owner_rent_amount', {
				...
				if amount == nil then
					...
					if closestPlayer == -1 or closestDistance > 5.0 then
						...
						vehicleProps.plate = newPlate
						SetVehicleNumberPlateText(LastVehicles[#LastVehicles], newPlate)
						TriggerServerEvent('ktr_truckshop:rentVehicle', model, vehicleProps.plate, GetPlayerName(closestPlayer), CurrentVehicleData.price, amount, GetPlayerServerId(closestPlayer))
					--	if Config.EnableJobLogs == true then
					--		TriggerServerEvent('esx_joblogs:AddInLog', 'vehicleshop', 'carbuysociety', GetPlayerName(closestPlayer), GetPlayerName(PlayerId()), newPlate, model)
					--	end
						if Config.EnableOwnedVehicles then
							TriggerServerEvent('ktr_truckshop:setVehicleOwnedPlayerId', GetPlayerServerId(closestPlayer), vehicleProps)
						end
						...
						TriggerServerEvent('ktr_truckshop:setVehicleForAllPlayers', vehicleProps, Config.Zones.ShopInside.Pos.x, Config.Zones.ShopInside.Pos.y, Config.Zones.ShopInside.Pos.z, 5.0)
					end
				end
			end, function (data2, menu2)
				menu2.close()
...
function OpenRentedVehiclesMenu()
	ESX.TriggerServerCallback('ktr_truckshop:getRentedVehicles', function (vehicles)
		local elements = {}
...
function OpenBossActionsMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'reseller',{
		...
		elseif data.current.value == 'sold_vehicles' then
			...
			ESX.TriggerServerCallback('ktr_truckshop:getSoldVehicles', function(customers)
				local elements = {
...
function OpenGetStocksMenu()
	ESX.TriggerServerCallback('ktr_truckshop:getStockItems', function (items)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu', {
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_get_item_count', {
				...
				if count == nil then
					...
					TriggerServerEvent('ktr_truckshop:getStockItem', itemName, count)
					menu2.close()
					menu.close()
					OpenGetStocksMenu()
				end
...
function OpenPutStocksMenu()
	ESX.TriggerServerCallback('ktr_truckshop:getPlayerInventory', function (inventory)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu', {
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_put_item_count', {
				...
				if count == nil then
					...
					TriggerServerEvent('ktr_truckshop:putStockItems', itemName, count)
					menu2.close()
					menu.close()
					OpenPutStocksMenu()
				end
...
AddEventHandler('ktr_truckshop:hasEnteredMarker', function (zone)
...
AddEventHandler('ktr_truckshop:hasExitedMarker', function (zone)
...
Citizen.CreateThread(function ()
	while true do
		...
		if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
			...
			TriggerEvent('ktr_truckshop:hasEnteredMarker', currentZone)
		end

		if not isInMarker and HasAlreadyEnteredMarker then
			HasAlreadyEnteredMarker = false
			TriggerEvent('ktr_truckshop:hasExitedMarker', LastZone)
		end
	end
end)
		...
		if CurrentAction == nil then
			...
			if IsControlJustReleased(0, Keys['E']) then
				...
				elseif CurrentAction == 'give_back_vehicle' then
					ESX.TriggerServerCallback('ktr_truckshop:giveBackVehicle', function(isRentedVehicle)
				...
				elseif CurrentAction == 'resell_vehicle' then
					ESX.TriggerServerCallback('ktr_truckshop:resellVehicle', function(vehicleSold)
