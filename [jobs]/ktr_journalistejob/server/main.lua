...
RegisterServerEvent('ktr_journalistejob:getStockItem')
AddEventHandler('ktr_journalistejob:getStockItem', function(itemName, count)
...
ESX.RegisterServerCallback('ktr_journalistejob:getStockItems', function(source, cb)
	TriggerEvent('esx_addoninventory:getSharedInventory', 'society_journaliste', function(inventory)
		cb(inventory.items)
	end)
end)

ESX.RegisterServerCallback('ktr_journalistejob:getPlayerInventory', function(source, cb)
	local xPlayer = ESX.GetPlayerFromId(source)
	local items   = xPlayer.inventory

	cb( { items = items } )
end)
RegisterServerEvent('ktr_journalistejob:putStockItems')
AddEventHandler('ktr_journalistejob:putStockItems', function(itemName, count)
	local xPlayer = ESX.GetPlayerFromId(source)
	local sourceItem = xPlayer.getInventoryItem(itemName)
...
ESX.RegisterServerCallback('ktr_journalistejob:buyJobVehicle', function(source, cb, vehicleProps, type)
	local xPlayer = ESX.GetPlayerFromId(source)
	local price = getPriceFromHash(vehicleProps.model, xPlayer.job.grade_name, type)
	local _source = source
	-- vehicle model not found
	if price == 0 then
		print(('ktr_journalistejob: %s attempted to exploit the shop! (invalid vehicle model)'):format(xPlayer.identifier))
		cb(false)
	else
		if xPlayer.getMoney() >= price then
			...
			--TriggerEvent('esx_joblogs:AddInLog', 'journalisteron', 'journalisteron_recolte1', xPlayer.name,amount, washedMoney)
...
ESX.RegisterServerCallback('ktr_journalistejob:storeNearbyVehicle', function(source, cb, nearbyVehicles)
	local xPlayer = ESX.GetPlayerFromId(source)
	...
	if not foundPlate then
		...
		MySQL.Async.execute('UPDATE owned_vehicles3 SET `stored` = true WHERE owner = @owner AND plate = @plate AND job = @job', {
			...
			if rowsChanged == 0 then
				print(('ktr_journalistejob: %s has exploited the garage!'):format(xPlayer.identifier))
				cb(false)
			else
				cb(true, foundNum)
			end
...
RegisterCommand("camera", function(source, args, raw)
    local src = source
    TriggerClientEvent("Cam:ToggleCam", src)
end)

RegisterCommand("microgrand", function(source, args, raw)
    local src = source
    TriggerClientEvent("Mic:ToggleBMic", src)
end)

RegisterCommand("micro", function(source, args, raw)
    local src = source
