...
RegisterServerEvent('esx_vigneronjob2:getStockItem')
AddEventHandler('esx_vigneronjob2:getStockItem', function(itemName, count)
...
ESX.RegisterServerCallback('esx_vigneronjob2:getStockItems', function(source, cb)
	TriggerEvent('esx_addoninventory:getSharedInventory', 'society_vigne', function(inventory)
		cb(inventory.items)
	end)
end)

ESX.RegisterServerCallback('esx_vigneronjob2:getPlayerInventory', function(source, cb)
	local xPlayer = ESX.GetPlayerFromId(source)
	local items   = xPlayer.inventory

	cb( { items = items } )
end)
RegisterServerEvent('esx_vigneronjob2:putStockItems')
AddEventHandler('esx_vigneronjob2:putStockItems', function(itemName, count)
	local xPlayer = ESX.GetPlayerFromId(source)
	local sourceItem = xPlayer.getInventoryItem(itemName)
...
ESX.RegisterServerCallback('esx_vigneronjob2:buyJobVehicle', function(source, cb, vehicleProps, type)
	local xPlayer = ESX.GetPlayerFromId(source)
	local price = getPriceFromHash(vehicleProps.model, xPlayer.job.grade_name, type)
	local _source = source
	-- vehicle model not found
	if price == 0 then
		print(('esx_vigneronjob2: %s attempted to exploit the shop! (invalid vehicle model)'):format(xPlayer.identifier))
		cb(false)
	else
...
ESX.RegisterServerCallback('esx_vigneronjob2:storeNearbyVehicle', function(source, cb, nearbyVehicles)
	local xPlayer = ESX.GetPlayerFromId(source)
	...
	if not foundPlate then
		...
		MySQL.Async.execute('UPDATE owned_vehicles3 SET `stored` = true WHERE owner = @owner AND plate = @plate AND job = @job', {
			...
			if rowsChanged == 0 then
				print(('esx_vigneronjob2: %s has exploited the garage!'):format(xPlayer.identifier))
				cb(false)
			else
				cb(true, foundNum)
			end
