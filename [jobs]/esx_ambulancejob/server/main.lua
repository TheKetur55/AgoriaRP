...
RegisterServerEvent('esx_ambulancejob:payFine2')
	AddEventHandler('esx_ambulancejob:payFine2', function()
		local xPlayer = ESX.GetPlayerFromId(source)
		local fineAmount = Config.EarlyRespawnFineAmount2
...
ESX.RegisterUsableItem('revivekit', function(source)
  ...
  if not hasAmbulance then
    TriggerClientEvent('esx_pharmacy:useDefibrillateur', source, 'revivekit')
    --xPlayer.removeInventoryItem(revivekit, math.random(0, 1))  
...
RegisterServerEvent('esx_pharmacy:revive')
AddEventHandler('esx_pharmacy:revive', function(target)
	local xPlayer = ESX.GetPlayerFromId(source)
		TriggerClientEvent('esx_pharmacy:revive', target)
end)
