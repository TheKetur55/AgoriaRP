...
function OnPlayerDeath()
	...
	StartDeathTimer()
	StartDistressSignal()
	ClearPedTasksImmediately(GetPlayerPed(-1))  -- 4555
...
function SendDistressSignal()
	...
	local text = "Alerte coma"
	ESX.ShowNotification(_U('distress_sent'))

	TriggerServerEvent("call:makeCall", "ambulance", {x=plyPos.x,y=plyPos.y,z=plyPos.z},text, GetPlayerServerId(player))
end
...
AddEventHandler('esx_pharmacy:useDefibrillateur', function(itemName)
  ...
  if closestPlayer == -1 or closestDistance > 3.0 then
    ...
    if health == 0 then
      local playerPed = GetPlayerPed(-1)
      Citizen.CreateThread(function()
        ESX.ShowNotification('Vous utiliser le defibrillateur')
        TaskStartScenarioInPlace(playerPed, 'CODE_HUMAN_MEDIC_TEND_TO_DEAD', 0, true)
        Citizen.Wait(15000)
        ClearPedTasks(playerPed)
        if GetEntityHealth(closestPlayerPed) == 0 then
          TriggerServerEvent('esx_pharmacy:revive', GetPlayerServerId(closestPlayer))
          ESX.ShowNotification('Vous avez relevez la personne')
        else
          ESX.ShowNotification('Impossible')
        end
      end)
    else
		  ESX.ShowNotification('La personne n\'est pas inconscient')
    end
  end
end)


RegisterNetEvent('esx_pharmacy:revive')
AddEventHandler('esx_pharmacy:revive', function()
	local playerPed = PlayerPedId()
	local coords = GetEntityCoords(playerPed)
