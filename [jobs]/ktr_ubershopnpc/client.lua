...
local pizzeria = { x = -1130.97, y = -1728.71, z = 3.47} --Commencer
local pizzeriafin = { x = -1152.44, y = -1740.44, z = 3.00} --Fın
local spawnfaggio = { x = -1152.44, y = -1740.44, z = 3.00} --Spawn véhicule
  ...
  while true do
    ...
    if isInJobPizz == false then
      ...
      if GetDistanceBetweenCoords(pizzeria.x, pizzeria.y, pizzeria.z, GetEntityCoords(GetPlayerPed(-1),true)) < 1.5 then
        HelpText("Appuyez sur ~INPUT_CONTEXT~ pour lancer la livraison de ~r~Uber Eat",0,1,0.5,0.8,0.6,255,255,255,255)

        if IsControlJustPressed(1,38) and PlayerData.job ~= nil and PlayerData.job.name == 'shop' then
            notif = true
            isInJobPizz = true
            isToHouse = true
            ...
            TriggerServerEvent("ktr_ubershopnpc:itemadd", nbPizza)
        end
      end
    end
    ...
    if isToHouse == true then
      ...
      while notif == true do
        ...
        ESX.ShowNotification('Livrer le Colis')
      ...
      if GetDistanceBetweenCoords(px,py,pz, GetEntityCoords(GetPlayerPed(-1),true)) < 3 then
        ...
        if IsControlJustPressed(1,38) then
          ...
          TriggerServerEvent("ktr_ubershopnpc:itemrm")
          nbPizza = nbPizza - 1
          ...
          if (posibilidad > 70) and (posibilidad < 90) then
            ...
            TriggerServerEvent("ktr_ubershopnpc:pourboire", pourboire)
    ...
    if isToPizzaria == true then
      ...
      while notif2 == true do
        ...
        ESX.ShowNotification('Direction le Uber Eat')
        ...
        if IsVehicleModel(GetVehiclePedIsIn(GetPlayerPed(-1), true), GetHashKey("faggio2"))  then
          ...
          if IsControlJustPressed(1,38) then
            ...
            if IsInVehicle() then
              ...
              TriggerServerEvent("ktr_ubershopnpc:pourboire", paie)
  ...
  while true do
    ...
    if isInJobPizz == true then
      ...
      if GetDistanceBetweenCoords(pizzeriafin.x, pizzeriafin.y, pizzeriafin.z, GetEntityCoords(GetPlayerPed(-1),true)) < 1.5 then
        ...
        if IsControlJustPressed(1,38) then
          TriggerServerEvent('ktr_ubershopnpc:deleteAllPizz')
          isInJobPizz = false
          livr = 0
          isToHouse = false
          isToPizzaria = false
          ...
          if afaitunepizzamin == true then
            ...
            TriggerServerEvent("ktr_ubershopnpc:paiefinale")
