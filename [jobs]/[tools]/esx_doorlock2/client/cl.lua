...
Citizen.CreateThread(function()
	...
	ESX.TriggerServerCallback('esx_doorlock2:getDoorInfo', function(doorInfo, count)
...
RegisterNetEvent('esx_doorlock2:currentlyhacking')
AddEventHandler('esx_doorlock2:currentlyhacking', function(mycb)
			mycb = true
			TriggerEvent("mhacking:show") --This line is where the hacking even starts
			TriggerEvent("mhacking:start",7,15,mycb1) --This line is the difficulty and tells it to start. First number is how long the blocks will be the second is how much time they have is.
end)
...
if success then
	...
	TriggerServerEvent('esx_doorlock2:updateState', i, doorID.locked)
	...
	print('Failure')
		TriggerEvent('mhacking:hide')
				...
				if IsControlJustReleased(0, Keys['G']) then
					TriggerServerEvent('esx_doorlock2:hack', mycb)
				end
				...
				if IsControlJustReleased(0, Keys['E']) then
					if isAuthorized then
						...
						TriggerServerEvent('esx_doorlock2:updateState', i, doorID.locked) -- Broadcast new state of the door to everyone
					end
				end
			end
		end
