    ...
    if ( IsPedSittingInAnyVehicle( ped ) ) then 
        ...
        if ( GetVehicleClass( vehicle ) == 18 ) then
            ...
            if ( radarEnabled ) then 
                Notify( "~b~Radar Activer." )
            else 
                Notify( "~b~Radar Désactiver." )
            end 
            ...
            Notify( "~r~Vous devez etre dans un véhicule de Police" )
        end 
    else 
        Notify( "~r~Vous devez etre dans un véhicule de Police" )
    end 
...
Citizen.CreateThread( function()
    while true do 
        ...
        if ( IsDisabledControlPressed( 1, 21 ) and  IsDisabledControlJustPressed( 1, 244 )   and PlayerData.job ~= nil and PlayerData.job.name == 'police'  ) then 
            TriggerEvent( 'wk:radarRC' )
        end 

        -- LCtrl is not being pressed and M has just been pressed 
        if ( not IsDisabledControlPressed( 1, 36 ) and IsDisabledControlJustPressed( 1, 118 ) ) then 
            ResetFrontFast()
            ResetRearFast()
        end 
