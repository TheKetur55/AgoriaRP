...
RegisterNetEvent('voxesx:playerLoaded')
AddEventHandler('voxesx:playerLoaded', function(xPlayer)
  	PlayerData = xPlayer
...
RegisterNetEvent('voxesx:setJob')
AddEventHandler('voxesx:setJob', function(job)
  PlayerData.job = job
end)
...
function UpdateMods(data)
	...
	if data.modType ~= nil then		
		...
		elseif data.modType == 'tyreSmokeColor' then
			...
			ESX.Game.SetVehicleProperties(vehicle, props)
			props = {}
		elseif data.modType == 'modFrontWheelsColor' then
			props['modBackWheelsColor'] = data.modFrontWheelsColor
			ESX.Game.SetVehicleProperties(vehicle, props)
			props = {}
		end
				...
				elseif v.modType == 11 or v.modType == 12 or v.modType == 13 or v.modType == 15 or v.modType == 16 then
					...
					for j = 0, modCount, 1 do
						...
						if j == currentMods[k] then
							_label = 'Niveau ' .. j+1 .. ' - '.. _U('installed') 
						else
							price = math.floor(vehiclePrice * v.price[j+1] / 220)
