			...
			if PlayerData.job ~= nil  then
				if(v.Type ~= -1 and GetDistanceBetweenCoords(coords, v.Pos.x, v.Pos.y, v.Pos.z, true) < Config.DrawDistance) then
					...
					if GetDistanceBetweenCoords(coords, v.Pos.x, v.Pos.y, v.Pos.z, true) < 1.5 then
						DrawText3D(v.Pos.x, v.Pos.y, v.Pos.z+0.9, "~w~[~g~E~w~] Gérer les caméra.", 0.80)
					end
				end
			end
		end
			...
			if Config.Hacking then
				if PlayerData.job ~= nil  then
					if(v.Type ~= -1 and GetDistanceBetweenCoords(coords, v.Pos.x, v.Pos.y, v.Pos.z, true) < Config.DrawDistance) then
						...
						if GetDistanceBetweenCoords(coords, v.Pos.x, v.Pos.y, v.Pos.z, true) < 1.5 then
							DrawText3D(v.Pos.x, v.Pos.y, v.Pos.z+0.9, "~w~[~g~E~w~] Hacker le systeme.", 0.80)
						end
					end
				end
			end
		...
		if IsControlJustReleased(0, Keys['E']) and CurrentAction == 'cameras' then
			...
			if not menuopen then
				...
				local elements = {
					...
					{label = ('Concession'), value = 'concess'},
					{label = ('Circuit Agoria Event'), value = 'circuit'},
					{label = ('Frontiere Los Santos'), value = 'frontiere'},
					{label = ('Place d\'Agoria'), value = 'agoria'}
				}
				...
				ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'cloakroom',
				{
					...
					elseif data.current.value == 'policemenu' then
						...
						elseif data.current.value == 'agoria' then

						if agoriaHacked then
							if Config.pNotify then
								TriggerEvent("pNotify:SendNotification",{
									text = _U('broken_cameras'),
									type = "warning",
							...
							agoriacamera = true
							blockbuttons = true
							local firstCamx = Config.Locations[1].agoriaCameras[1].x
							local firstCamy = Config.Locations[1].agoriaCameras[1].y
							local firstCamz = Config.Locations[1].agoriaCameras[1].z
							local firstCamr = Config.Locations[1].agoriaCameras[1].r
							...
							SendNUIMessage({
								type  = "enablecam",
								label = Config.Locations[1].agoriaCameras[1].label,
								box   = Config.Locations[1].agoriaCameras.label
							})
...
elseif data.current.value == 'circuit' then

						if circuitHacked then
							if Config.pNotify then
								TriggerEvent("pNotify:SendNotification",{
									text = _U('broken_cameras'),
									type = "warning",
							...
							circuitcamera = true
							blockbuttons = true
							local firstCamx = Config.Locations[1].circuitCameras[1].x
							local firstCamy = Config.Locations[1].circuitCameras[1].y
							local firstCamz = Config.Locations[1].circuitCameras[1].z
							local firstCamr = Config.Locations[1].circuitCameras[1].r
							...
							SendNUIMessage({
								type  = "enablecam",
								label = Config.Locations[1].circuitCameras[1].label,
								box   = Config.Locations[1].circuitCameras.label
							})
...
elseif data.current.value == 'concess' then

						if concessHacked then
							if Config.pNotify then
								TriggerEvent("pNotify:SendNotification",{
									text = _U('broken_cameras'),
									type = "warning",
							...
							concesscamera = true
							blockbuttons = true
							local firstCamx = Config.Locations[1].concessCameras[1].x
							local firstCamy = Config.Locations[1].concessCameras[1].y
							local firstCamz = Config.Locations[1].concessCameras[1].z
							local firstCamr = Config.Locations[1].concessCameras[1].r
							...
							SendNUIMessage({
								type  = "enablecam",
								label = Config.Locations[1].concessCameras[1].label,
								box   = Config.Locations[1].concessCameras.label
							})
						...
						elseif data.current.value == 'frontiere' then

						if frontiereHacked then
							if Config.pNotify then
								TriggerEvent("pNotify:SendNotification",{
									text = _U('broken_cameras'),
									type = "warning",
							...
							menu.close()
							frontierecamera = true
							blockbuttons = true
							local firstCamx = Config.Locations[1].frontiereCameras[1].x
							local firstCamy = Config.Locations[1].frontiereCameras[1].y
							local firstCamz = Config.Locations[1].frontiereCameras[1].z
							local firstCamr = Config.Locations[1].frontiereCameras[1].r
							...
							SendNUIMessage({
								type  = "enablecam",
								label = Config.Locations[1].frontiereCameras[1].label,
								box   = Config.Locations[1].frontiereCameras.label
							})
		...
		if createdCamera ~= 0 then
			...
			if IsControlJustPressed(0, Keys["LEFT"]) then
				...
				elseif policecamera then
					...
					elseif agoriacamera then
					local newCamIndex

					if currentCameraIndexIndex == 1 then
						newCamIndex = #Config.Locations[currentCameraIndex].agoriaCameras
					else
						newCamIndex = currentCameraIndexIndex - 1
					end

					local newCamx = Config.Locations[currentCameraIndex].agoriaCameras[newCamIndex].x
					local newCamy = Config.Locations[currentCameraIndex].agoriaCameras[newCamIndex].y
					local newCamz = Config.Locations[currentCameraIndex].agoriaCameras[newCamIndex].z
					local newCamr = Config.Locations[currentCameraIndex].agoriaCameras[newCamIndex].r

					SetFocusArea(newCamx, newCamy, newCamz, newCamx, newCamy, newCamz)
					SendNUIMessage({
						type = "updatecam",
						label = Config.Locations[currentCameraIndex].agoriaCameras[newCamIndex].label
					})
				...
				--
					...
					elseif frontierecamera then
					local newCamIndex

					if currentCameraIndexIndex == 1 then
						newCamIndex = #Config.Locations[currentCameraIndex].frontiereCameras
					else
						newCamIndex = currentCameraIndexIndex - 1
					end

					local newCamx = Config.Locations[currentCameraIndex].frontiereCameras[newCamIndex].x
					local newCamy = Config.Locations[currentCameraIndex].frontiereCameras[newCamIndex].y
					local newCamz = Config.Locations[currentCameraIndex].frontiereCameras[newCamIndex].z
					local newCamr = Config.Locations[currentCameraIndex].frontiereCameras[newCamIndex].r

					SetFocusArea(newCamx, newCamy, newCamz, newCamx, newCamy, newCamz)
					SendNUIMessage({
						type = "updatecam",
						label = Config.Locations[currentCameraIndex].frontiereCameras[newCamIndex].label
					})

					ChangeSecurityCamera(newCamx, newCamy, newCamz, newCamr)
					currentCameraIndexIndex = newCamIndex

					elseif concesscamera then
					local newCamIndex

					if currentCameraIndexIndex == 1 then
						newCamIndex = #Config.Locations[currentCameraIndex].concessCameras
					else
						newCamIndex = currentCameraIndexIndex - 1
					end

					local newCamx = Config.Locations[currentCameraIndex].concessCameras[newCamIndex].x
					local newCamy = Config.Locations[currentCameraIndex].concessCameras[newCamIndex].y
					local newCamz = Config.Locations[currentCameraIndex].concessCameras[newCamIndex].z
					local newCamr = Config.Locations[currentCameraIndex].concessCameras[newCamIndex].r

					SetFocusArea(newCamx, newCamy, newCamz, newCamx, newCamy, newCamz)
					SendNUIMessage({
						type = "updatecam",
						label = Config.Locations[currentCameraIndex].concessCameras[newCamIndex].label
					})

					ChangeSecurityCamera(newCamx, newCamy, newCamz, newCamr)
					currentCameraIndexIndex = newCamIndex

					elseif circuitcamera then
					local newCamIndex

					if currentCameraIndexIndex == 1 then
						newCamIndex = #Config.Locations[currentCameraIndex].circuitCameras
					else
						newCamIndex = currentCameraIndexIndex - 1
					end

					local newCamx = Config.Locations[currentCameraIndex].circuitCameras[newCamIndex].x
					local newCamy = Config.Locations[currentCameraIndex].circuitCameras[newCamIndex].y
					local newCamz = Config.Locations[currentCameraIndex].circuitCameras[newCamIndex].z
					local newCamr = Config.Locations[currentCameraIndex].circuitCameras[newCamIndex].r

					SetFocusArea(newCamx, newCamy, newCamz, newCamx, newCamy, newCamz)
					SendNUIMessage({
						type = "updatecam",
						label = Config.Locations[currentCameraIndex].circuitCameras[newCamIndex].label
					})
			...
			if IsControlJustPressed(0, Keys["RIGHT"]) then
				...
				elseif policecamera then
					...
					if currentCameraIndexIndex == #Config.Locations[currentCameraIndex].frontiereCameras then
						newCamIndex = 1
					else
						newCamIndex = currentCameraIndexIndex + 1
					end
					...
					if currentCameraIndexIndex == #Config.Locations[currentCameraIndex].concessCameras then
						newCamIndex = 1
					else
						newCamIndex = currentCameraIndexIndex + 1
					end
				...
				elseif circuitcamera then
					...
					if currentCameraIndexIndex == #Config.Locations[currentCameraIndex].circuitCameras then
						newCamIndex = 1
					else
						newCamIndex = currentCameraIndexIndex + 1
					end
				...
				elseif agoriacamera then
					...
					if currentCameraIndexIndex == #Config.Locations[currentCameraIndex].agoriaCameras then
						newCamIndex = 1
					else
						newCamIndex = currentCameraIndexIndex + 1
					end
