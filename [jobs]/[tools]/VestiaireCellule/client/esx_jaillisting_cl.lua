...
function ShowJailListingMenu(data)
		...
		ESX.UI.Menu.Open(
			'default', GetCurrentResourceName(), 'jaillisting',
			{
				...
				{label = ('Tenue Photo'), value = 'jail_wear'},
				{label = ('Tenue Cellule'), value = 'jail_wear2'},
			},
			},
			function(data, menu)
			...
			if data.current.value == 'jail_wear' then 
				...
				ESX.TriggerServerCallback('esx_skin:getPlayerSkin', function(skin, jailSkin)
					if skin.sex == 0 then
						SetPedComponentVariation(GetPlayerPed(-1), 3, 15, 0, 0)--Bras
						SetPedComponentVariation(GetPlayerPed(-1), 4, 14, 1, 0)--Jean
						SetPedComponentVariation(GetPlayerPed(-1), 6, 42, 1, 0)--Chaussure
						SetPedComponentVariation(GetPlayerPed(-1), 11, 91, 0, 0)-- Torse
						SetPedComponentVariation(GetPlayerPed(-1), 8, 57, 0, 0)--TEE SHIRT
						SetPedComponentVariation(GetPlayerPed(-1), 5, 0, 0, 0) -- SAC A DOS
						SetPedComponentVariation(GetPlayerPed(-1), 1, -1, 0, 0)  -- MASQUE
						SetPedComponentVariation(GetPlayerPed(-1), 7, 0, 0, 0)  -- CHAINE
					...
					elseif skin.sex == 1 then
                        ...
                        SetPedComponentVariation(GetPlayerPed(-1), 11, 91, 0, 0)--Veste
                        SetPedComponentVariation(GetPlayerPed(-1), 8, 0, 0, 0)--GiletJaune
					else
			...
			if data.current.value == 'jail_wear2' then 
