  ...
  function Start(ped)
	  ...
	  while checking do
		  ...
		  if distance < 2.0 then
			  DrawText3D(x,y,z, 'Appuyez sur [~g~E~s~] pour connaitre la cause du déces', 0.4)
  ...
  function Notification(x,y,z)
	  ...
	  while (timestamp + 4500) > GetGameTimer() do
		  Citizen.Wait(0)
		  DrawText3D(x, y, z, 'Les dégâts semblent venir d\'ici', 0.4)
		  checking = false
	  end
  end
  ...
  function OpenDeathMenu(player)
	  ...
	  table.insert(elements, {label = 'Essayez d\'identifier la cause de la mort', value = 'deathcause'})
	  table.insert(elements, {label = 'Essayez d\'identifier l\'endroit du dommage', value = 'damage'})
  
  
	  ESX.UI.Menu.Open(
		  'default', GetCurrentResourceName(), 'dead_citizen',
		  {
			  title    = 'AGORIA PARADISE',
			  align    = 'top-left',
			  elements = elements,
		  },
	  function(data, menu)
		  ...
		  if ac == 'damage' then
			  ...
			  if success then
				  ...
				  Notify('Le lieu où le dommage est survenu n\'a pas pu être identifié')
			  end
		  end
