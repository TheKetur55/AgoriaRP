...
local iaa_hash = GetHashKey("valkyrie")
local fib_hash = GetHashKey("frogger2")
local buzzard_hash = GetHashKey("buzzard2")
local buzzard2_hash = GetHashKey("buzzard")
local iaa2_hash = GetHashKey("valkyrie")
local fov = (fov_max+fov_min)*0.5
local vision_state = 0 -- 0 is normal, 1 is nightmode, 2 is thermal vision
Citizen.CreateThread(function()
	while true do
        Citizen.Wait(0)
		if IsPlayerInPolmav() or IsPlayerInIaa() or IsPlayerInIaa2() or IsPlayerInFib() then
...
function IsPlayerBuzzard2()
	local lPed = GetPlayerPed(-1)
	local vehicle = GetVehiclePedIsIn(lPed)
	return IsVehicleModel(vehicle, buzzard2_hash)
end

function IsPlayerBuzzard()
	local lPed = GetPlayerPed(-1)
	local vehicle = GetVehiclePedIsIn(lPed)
	return IsVehicleModel(vehicle, buzzard_hash)
end

function IsPlayerInIaa()
	local lPed = GetPlayerPed(-1)
	local vehicle = GetVehiclePedIsIn(lPed)
	return IsVehicleModel(vehicle, iaa_hash)
end

function IsPlayerInIaa2()
	local lPed = GetPlayerPed(-1)
	local vehicle = GetVehiclePedIsIn(lPed)
	return IsVehicleModel(vehicle, iaa2_hash)
end

function IsPlayerInFib()
	local lPed = GetPlayerPed(-1)
	local vehicle = GetVehiclePedIsIn(lPed)
	return IsVehicleModel(vehicle, fib_hash)
end
