...
AddEventHandler('esx:playerLoaded', function(xPlayer)
	PlayerData = xPlayer
	ESX.TriggerServerCallback('ktr_agotuning:getVehiclesPrices', function(vehicles)
		Vehicles = vehicles
	end)
end)

RegisterNetEvent('esx:setSecondJob')
AddEventHandler('esx:setSecondJob', function(second_job)
	PlayerData.second_job = second_job
end)

RegisterNetEvent('ktr_agotuning:installMod')
AddEventHandler('ktr_agotuning:installMod', function()
	local vehicle = GetVehiclePedIsIn(PlayerPedId(), false)
	myCar = ESX.Game.GetVehicleProperties(vehicle)
	TriggerServerEvent('ktr_agotuning:refreshOwnedVehicle', myCar)
end)

RegisterNetEvent('ktr_agotuning:cancelInstallMod')
AddEventHandler('ktr_agotuning:cancelInstallMod', function()
	local vehicle = GetVehiclePedIsIn(PlayerPedId(), false)
...
function OpenLSMenu(elems, menuName, menuTitle, parent)
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), menuName,
	{
		...
		for k,v in pairs(Config.Menus) do
			...
			if k == data.current.modType or isRimMod then
				...
				if data.current.label == _U('by_default') or string.match(data.current.label, _U('installed')) then
					...
					TriggerEvent('ktr_agotuning:installMod')
				else
					...
					if isRimMod then
						price = math.floor(vehiclePrice * data.current.price / 220)
						TriggerServerEvent("ktr_agotuning:buyMod", price)
						if Config.EnableJobLogs then
					--		TriggerServerEvent('esx_joblogs:AddInLog', "mecano", "buy_mod", GetPlayerName(PlayerId()), price, vehicleData.plate, VehicleName)
						end
					...
					elseif v.modType == 17 then
						price = math.floor(vehiclePrice * v.price[1] / 220)
						TriggerServerEvent("ktr_agotuning:buyMod", price)
						if Config.EnableJobLogs then
						--	TriggerServerEvent('esx_joblogs:AddInLog', "mecano", "buy_mod", GetPlayerName(PlayerId()), price, vehicleData.plate, VehicleName)
						end
					else
						price = math.floor(vehiclePrice * v.price / 220)
						TriggerServerEvent("ktr_agotuning:buyMod", price)
						if Config.EnableJobLogs then
						--	TriggerServerEvent('esx_joblogs:AddInLog', "mecano", "buy_mod", GetPlayerName(PlayerId()), price, vehicleData.plate, VehicleName)
						end
		...
		menu.close()
		TriggerEvent('ktr_agotuning:cancelInstallMod')
...
Citizen.CreateThread(function()
	while true do
		...
		if IsPedInAnyVehicle(playerPed, false) then
			...
			local zone 		  = nil
			local lastZone    = nil
			if PlayerData.second_job ~= nil and PlayerData.second_job.name == 'blackgarage' or Config.IsmechanicJobOnly == false then
				for k,v in pairs(Config.Zones) do
			...
			if IsControlJustReleased(0, Keys['G']) and not lsMenuIsShowed and isInLSMarker then
				if PlayerData.second_job ~= nil and PlayerData.second_job.name == 'blackgarage' or Config.IsmechanicJobOnly == false then
					lsMenuIsShowed = true
