...
function StartJob(jobid)
	if jobid == 1 then -- taxi
		showLoadingPromt("Chargement des missions LSPD", 2000, 3)
		jobs.coords.cx[1],jobs.coords.cy[1],jobs.coords.cz[1] = 394.94,-981.55,27.99
		jobs.coords.cx[2],jobs.coords.cy[2],jobs.coords.cz[2] = 368.22,-1571.23,27.99
		jobs.coords.cx[3],jobs.coords.cy[3],jobs.coords.cz[3] = 822.66,-1267.62,24.99
		jobs.coords.cx[4],jobs.coords.cy[4],jobs.coords.cz[4] = 621.07,31.43,87.90
		jobs.coords.cx[5],jobs.coords.cy[5],jobs.coords.cz[5] = 1854.61,2586.02,44.99
		--[[
		...
		Wait(2000)
		DrawMissionText("Faites un tour pour trouver un ~h~~y~Suspect~w~.", 10000)
		onJob = jobid
	end
end
				...
				if IsPedSittingInVehicle(GetPlayerPed(-1), jobs.cars[1]) then
					if DoesEntityExist(jobs.peds[1]) then
						if IsPedFatallyInjured(jobs.peds[1]) then
							...
							DrawMissionText("Le Suspect est ~r~mort~w~. Trouvez en un autre.", 5000)
						else
							if jobs.flag[1] == 1 and jobs.flag[2] > 0 then
								...
								if jobs.flag[2] == 0 then
									...
									Citizen.InvokeNative(0xB736A491E64A32CF,Citizen.PointerValueIntInitialized(jobs.peds[1]))
									jobs.peds[1] = nil
									DrawMissionText("Le Suspect ~r~c'est enfuit~w~. Trouvez en un autre.", 5000)
									jobs.flag[1] = 0
									jobs.flag[2] = 59+GetRandomIntInRange(1, 61)
								else
									if IsPedSittingInVehicle(GetPlayerPed(-1), jobs.cars[1]) then
							...
							if jobs.flag[1] == 2 and jobs.flag[2] > 0 then
								...
								if jobs.flag[2] == 0 then
									...
									Citizen.InvokeNative(0xB736A491E64A32CF,Citizen.PointerValueIntInitialized(jobs.peds[1]))
									jobs.peds[1] = nil
									DrawMissionText("~r~Le suspect s'est enfuit ~w~. Trouvez en un autre.", 5000)
									jobs.flag[1] = 0
									jobs.flag[2] = 59+GetRandomIntInRange(1, 61)
								else
									if IsPedSittingInVehicle(jobs.peds[1], jobs.cars[1]) then
										...
										if street[2] ~= 0 and street[2] ~= nil then
											local streetname = string.format("~w~Emmenez le suspect à ~y~ %s~w~, proche de ~y~ %s", GetStreetNameFromHashKey(street[1]),GetStreetNameFromHashKey(street[2]))
											DrawMissionText(streetname, 5000)
										else
											local streetname = string.format("~w~Emmenez le suspect à ~y~ %s", GetStreetNameFromHashKey(street[1]))
											DrawMissionText(streetname, 5000)
										end
                  ...
                  DrawMissionText("~g~Le suspect est bien arrivé !", 5000)
									-- pay money on something
									Wait(8000)
									DrawMissionText("Faites un tour pour trouver un nouveau ~h~~y~suspect~w~.", 10000)
									jobs.flag[1] = 0
									jobs.flag[2] = 59+GetRandomIntInRange(1, 90)
								end
							end
						...
						if jobs.flag[1] == 0 and jobs.flag[2] > 0 then
							...
							if jobs.flag[2] == 0 then
								...
								if DoesEntityExist(rped) then
									...
									DrawMissionText("Le ~g~suspect~w~ vous attend. Allez-y", 5000)
