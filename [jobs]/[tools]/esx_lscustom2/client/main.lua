				...
				if k == data.current.modType or isRimMod then
					if data.current.label == _U('by_default') or string.match(data.current.label, _U('installed')) then
						...
						elseif v.modType == 11 or v.modType == 12 or v.modType == 13 or v.modType == 15 or v.modType == 16 then
							price = math.floor(vehiclePrice * v.price[data.current.modNum + 1] / 120)
							TriggerServerEvent("esx_lscustom2:buyMod", price)
						elseif v.modType == 17 then
							price = math.floor(vehiclePrice * v.price[1] / 115)
							TriggerServerEvent("esx_lscustom2:buyMod", price)
						else
							price = math.floor(vehiclePrice * v.price / 115)
							TriggerServerEvent("esx_lscustom2:buyMod", price)
						end
			...
			if v.modType ~= nil then
				...
				elseif v.modType == 11 or v.modType == 12 or v.modType == 13 or v.modType == 15 or v.modType == 16 then
					...
					for j = 0, modCount, 1 do
						...
						if j == currentMods[k] then
							...
							price = math.floor(vehiclePrice * v.price[j+1] / 115)
				...
				elseif v.modType == 17 then -- TURBO
					...
					if currentMods[k] then
						...
						_label = 'Turbo - <span style="color:green;">$' .. math.floor(vehiclePrice * v.price[1] / 115) .. ' </span>'
					end
					table.insert(elements, {label = _label, modType = k, modNum = true})
				else
...
Citizen.CreateThread(function()
	while true do
		...
		if IsPedInAnyVehicle(playerPed, false) then
			...
			local zone 		  = nil
			local lastZone    = nil
			if (PlayerData.second_job ~= nil and PlayerData.second_job.name == 'iaa') or Config.IsMecanoJobOnly == false then
