...
function OpenCloakroomMenu()
	...
	elseif grade == 'juge' then
	    table.insert(elements, {label = 'Tenue Juge', value = 'juge_wear'})
	...
	elseif grade == 'GSI' then
		table.insert(elements, {label = _U('police_wear'), value = 'GSI_wear'})
	...
	if PlayerData.job.grade >= 4 then
    	table.insert(elements, {label = 'Tenue Pilote', value = 'helicopterpilot_wear'})
  	end
  	if PlayerData.job.grade >= 2 then
    	table.insert(elements, {label = 'Tenue Motard', value = 'motorcyclist_wear'})
  	end
  	if PlayerData.job.grade >= 2 then
    	table.insert(elements, {label = 'Tenue Investigation', value = 'investigator_wear'})
  	end
  	if PlayerData.job.grade >= 1 then
    	table.insert(elements, {label = 'Tenue Intervention', value = 'swat_wear'})
  	end
  	if PlayerData.job.grade >= 1 then
    	table.insert(elements, {label = 'Tenue enquête', value = 'enquete_wear'})
  	end
  	if PlayerData.job.grade >= 4 then
    	table.insert(elements, {label = 'Tenue BRI', value = 'highway_wear'})
  	end
...
function OpenArmoryMenu(station)
 ...
 if onDuty then
	...
	if Config.EnablePlayerManagement and PlayerData.job.grade_name == 'boss' or PlayerData.job.grade_name == 'chef' or PlayerData.job.grade_name == 'officer' or PlayerData.job.grade_name == 'sergeant2' or PlayerData.job.grade_name == 'sergeant' or PlayerData.job.grade_name == 'lieutenant' or PlayerData.job.grade_name == 'GSI' then
...
function OpenPoliceActionsMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'police_actions',
	{
		...
		if data.current.value == 'alerte_lspd' then
			...
			local text = GetPlayerName(PlayerId()).." fait une alerte LSPD"
            TriggerServerEvent("call:makeCall", "police", {x=plyPos.x,y=plyPos.y,z=plyPos.z},text, GetPlayerServerId(player))
			end
          ...
          function(data, menu)
            ...
            if amount == nil or amount < 0 then
              ...
              if closestPlayer == -1 or closestDistance > 3.0 then
                ...
                TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(closestPlayer), 'society_police', ('Facture LSPD : '), amount)
              end
            end
          end,
        function(data, menu)
...
function ShowPlayerLicense2(player)
	...
	ESX.TriggerServerCallback('esx_policejob:getOtherPlayerData', function(data)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'manage_license',
		{
			...
			TriggerServerEvent('esx_policejob:message', GetPlayerServerId(player), _U('license_revoked2', data.current.label))
...
function OpenGetWeaponMenu2()
	...
	ESX.TriggerServerCallback('esx_policejob:getArmoryWeapons2', function(weapons)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'armory_get_weapon2',
		{
			...
			ESX.TriggerServerCallback('esx_policejob:removeArmoryWeapon2', function()
				OpenGetWeaponMenu2()
			end, data.current.value)
...
function OpenPutWeaponMenu2()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'armory_put_weapon2',
	{
		...
		ESX.TriggerServerCallback('esx_policejob:addArmoryWeapon2', function()
			OpenPutWeaponMenu2()
		end, data.current.value, true)
...
AddEventHandler('esx_policejob:updateBlip', function()
	...
	if PlayerData.job ~= nil and PlayerData.job.name == 'police' and onDuty then  --
		    ...
		    while true do
		        Citizen.Wait(1)    
		            ...
		            if (affichenews == true) then
				            ...
				            DrawAdvancedTextCNN(0.588, 0.02, 0.005, 0.0028, 0.8, "~r~ LSPD ~d~", 255, 255, 255, 255, 1, 0)
