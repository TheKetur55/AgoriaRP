...
function Message2()
    ...
    while messagenotfinish do
		    ...
		    if (GetOnscreenKeyboardResult()) then
		        local result = GetOnscreenKeyboardResult()
		        messagenotfinish = false
		       TriggerServerEvent('esx_mecanojob:annonce2',result)
...
--      table.insert(elements, {label = _U('boss_actions'), value = 'boss_actions'})
 --   end
    ...
    if PlayerData.job.grade_name == 'novice' then
      table.insert(elements, {label = _U('work_wear'), value = 'novice_wear'})
    end

    if PlayerData.job.grade_name == 'experimente' then
      table.insert(elements, {label = _U('work_wear'), value = 'novice_wear'})
    end
			...
			if Config.MaxInService ~= -1 then
				...
				ESX.TriggerServerCallback('esx_service:isInService', function(isInService)
					if isInService then
						...
						TriggerServerEvent('esx_service:notifyAllInService', notification, 'mecano')

						TriggerServerEvent('esx_service:disableService', 'mecano')
						TriggerEvent('esx_mecanojob:updateBlip')
...
function OpenVehicleSpawnerMenu(type, station, part, partNum)
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle', {
		...
		if data.current.action == 'buy_vehicle' then
			...
			if type == 'car' then
				shopCoords = Config.mecanoStations[station].Vehicles[partNum].InsideShop
			...
			elseif type == 'helicopter' then
				shopCoords = Config.mecanoStations[station].Helicopters[partNum].InsideShop
...
function StoreNearbyVehicle(playerCoords)
	...
	ESX.TriggerServerCallback('esx_mecanojob:storeNearbyVehicle', function(storeSuccess, foundNum)
...
function GetAvailableVehicleSpawnPoint(station, part, partNum)
	local spawnPoints = Config.mecanoStations[station][part][partNum].SpawnPoints
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_shop', {
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_shop_confirm',
		{
			...
			if data2.current.value == 'yes' then
				...
				ESX.TriggerServerCallback('esx_mecanojob:buyJobVehicle', function (bought)
					if bought then
...
function OpenmecanoActionsMenu2()
	ESX.UI.Menu.CloseAll()
...
function OpenBuyWeaponsMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'armory_buy_weapons', {
		...
		if data.current.hasWeapon then
			...
			ESX.TriggerServerCallback('esx_mecanojob:buyWeapon', function(bought)
				if bought then
...
AddEventHandler('esx_phone:cancelMessage', function(dispatchNumber)
	if type(PlayerData.job.name) == 'string' and PlayerData.job.name == 'mecano' and PlayerData.job.name == dispatchNumber then
...
AddEventHandler('esx_mecanojob:hasEnteredMarker', function(station, part, partNum)
...
AddEventHandler('esx_mecanojob:hasExitedMarker', function(station, part, partNum)
...
RegisterNetEvent('esx_mecanojob:handcuff')
AddEventHandler('esx_mecanojob:handcuff', function()
...
RegisterNetEvent('esx_mecanojob:unrestrain')
AddEventHandler('esx_mecanojob:unrestrain', function()
	if IsHandcuffed then
		local playerPed = PlayerPedId()
		IsHandcuffed = false
...
RegisterNetEvent('esx_mecanojob:drag')
AddEventHandler('esx_mecanojob:drag', function(copID)
...
RegisterNetEvent('esx_mecanojob:putInVehicle')
AddEventHandler('esx_mecanojob:putInVehicle', function()
	local playerPed = PlayerPedId()
	local coords = GetEntityCoords(playerPed)
...
RegisterNetEvent('esx_mecanojob:OutVehicle')
AddEventHandler('esx_mecanojob:OutVehicle', function()
	local playerPed = PlayerPedId()
...
Citizen.CreateThread(function()
	while true do
		...
		if IsControlJustReleased(0, Keys['F6']) and not isDead and PlayerData.job ~= nil and PlayerData.job.name == 'mecano' and not ESX.UI.Menu.IsOpen('default', GetCurrentResourceName(), 'mecano_actions') then
			if onDuty then
...
--[[
		if IsControlJustReleased(0, Keys['F6']) and not isDead and PlayerData.job ~= nil and PlayerData.job.name == 'agocustom'  and not ESX.UI.Menu.IsOpen('default', GetCurrentResourceName(), 'mecano_actions') then
		--	if onDuty then
				OpenmecanoActionsMenu2()
		--	else
		--		ESX.ShowNotification('Vous n\'etes pas en ~r~service')
		--	end
		end
		if IsControlJustReleased(0, Keys['F6']) and not isDead and PlayerData.job ~= nil and  PlayerData.job.name == 'agotuning' and not ESX.UI.Menu.IsOpen('default', GetCurrentResourceName(), 'mecano_actions') then
		--	if onDuty then
				OpenmecanoActionsMenu2()
		--	else
		--		ESX.ShowNotification('Vous n\'etes pas en ~r~service')
...
AddEventHandler('playerSpawned', function(spawn)
	isDead = false
	TriggerEvent('esx_mecanojob:unrestrain')
...
AddEventHandler('onResourceStop', function(resource)
	if resource == GetCurrentResourceName() then
		TriggerEvent('esx_mecanojob:unrestrain')
		TriggerEvent('esx_phone:removeSpecialContact', 'mecano')
		    ...
		    while true do
		        Citizen.Wait(1)    
		            ...
		            if (affichenews == true) then
				            ...
				            DrawAdvancedTextCNN(0.588, 0.02, 0.005, 0.0028, 0.8, "~r~ BENNY'S ~d~", 255, 255, 255, 255, 1, 0)
...
RegisterNetEvent('esx_mecanojob:annonce2')
AddEventHandler('esx_mecanojob:annonce2', function(text)
 		texteafiche = text
 		affichenews2 = true
...
RegisterNetEvent('esx_mecanojob:annoncestop2')
AddEventHandler('esx_mecanojob:annoncestop2', function()
 		affichenews2 = false
