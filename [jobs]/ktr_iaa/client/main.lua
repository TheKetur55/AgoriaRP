...
function OpenCloakroomMenu()
	...
	local grade = PlayerData.second_job.grade_name

	local elements = {
		{ label = _U('citizen_wear'), value = 'citizen_wear' },
		--{ label = ('Tenue Swat'), value = 'swat_npc' },
	...
	--	table.insert(elements, {label = _U('police_wear'), value = 'chef_wear'})
		table.insert(elements, {label = ('tenue de sortie'), value = 'work_wear'})
		table.insert(elements, {label = 'tenue classe', value = 'smocking_wear'})
		table.insert(elements, {label = ('Tenue d\'intervention 1'), value = 'swat2_wear'})
		table.insert(elements, {label = ('Tenue sniper'), value = 'swat_wear'})
		table.insert(elements, {label = 'Tenu EMS', value = 'fibems_wear'})
		table.insert(elements, {label = 'Tenu Police', value = 'fibpolice_wear'})
		table.insert(elements, {label = 'Tenu Pilote heli', value = 'helicopterpilot_wear'})
...
if data.current.value == 'citizen_wear' then
        ...
        TriggerServerEvent("player:serviceOff", "fib")
	...
	--[[	if data.current.value == 'citizen_wear' then
			...
			if Config.MaxInService ~= -1 then
				...
				ESX.TriggerServerCallback('esx_service:isInService', function(isInService)
					if isInService then
						...
						TriggerServerEvent('esx_service:notifyAllInService', notification, 'iaa')

						TriggerServerEvent('esx_service:disableService', 'iaa')
						TriggerEvent('ktr_iaa:updateBlip')
		...
		if
			...
			data.current.value == 'work_wear' or
			...
			data.current.value == 'fibpolice_wear' or
			data.current.value == 'smocking_wear' or
			data.current.value == 'helicopterpilot_wear' or
			data.current.value == 'fibems_wear' or
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle', {
		...
		if data.current.action == 'buy_vehicle' then
			...
			if type == 'car' then
				...
				local authorizedVehicles = Config.AuthorizedVehicles[PlayerData.second_job.grade_name]
			...
			elseif type == 'helicopter' then
				...
				local authorizedHelicopters = Config.AuthorizedHelicopters[PlayerData.second_job.grade_name]
		...
		elseif data.current.action == 'garage' then
			...
			ESX.TriggerServerCallback('esx_vehicleshop:retrieveJobVehicles2', function(jobVehicles)
...
function StoreNearbyVehicle(playerCoords)
	...
	ESX.TriggerServerCallback('ktr_iaa:storeNearbyVehicle', function(storeSuccess, foundNum)
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_shop', {
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_shop_confirm',
		{
			...
			if data2.current.value == 'yes' then
				...
				ESX.TriggerServerCallback('ktr_iaa:buyJobVehicle', function (bought)
					if bought then
...
function OpenPoliceActionsMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'police_actions',
	{
		...
		elements = {
			...
			{label = 'Intéractions arme',	value = 'weapon_interaction'},
			{label = _U('object_spawner'),		value = 'object_spawner'},
			{label = ('Montrer sa carte FIB'),	value = 'menuperso_moi_idcard_show'},
	...
	if  data.current.value == 'menuperso_moi_idcard_show' then
       local player, distance = ESX.Game.GetClosestPlayer()
        	if distance ~= -1 and distance <= 3.0 then
        	  TriggerServerEvent('fbibadge:open', GetPlayerServerId(PlayerId()), GetPlayerServerId(player))
        	else
      	...
      	if data.current.value == 'jail_menu' then
        	TriggerEvent("esx-qalle-jail:openJail1Menu")
        end

        if data.current.value == 'canon' then
        	TriggerEvent("OCmenu:OnMenuOpen")
        end
		...
		--	Citizen.Wait(10000)
		--	ESX.ShowNotification('~r~Nitro terminer!')
		--	SetVehicleBoostActive(playerVeh, 0, 0)
		----	SetVehicleForwardSpeed(playerVeh, force)
		--	StartScreenEffect("RaceTurbo", 0, 0)
		--	SetVehicleBoostActive(playerVeh, 0, 0)
 ...
 if data.current.value == 'deve' then
        	local veh = GetVehiclePedIsUsing(PlayerPedId())
ToggleVehicleMod(veh, 22, false) -- toggle xenon
SetVehicleHeadlightsColour(veh, 2)
        end
         if data.current.value == 'dev' then
        	local veh = GetVehiclePedIsUsing(PlayerPedId())
ToggleVehicleMod(veh, 22, true) -- toggle xenon
SetVehicleHeadlightsColour(veh, 2)
        end
        if data.current.value == 'devd' then
        	local veh = GetVehiclePedIsUsing(PlayerPedId())
ToggleVehicleMod(veh, 22, true) -- toggle xenon
SetVehicleHeadlightsColour(veh, 0)
        end

        if data.current.value == 'weapon_interaction' then
			local elements = {
				{label = 'Equiper Accessoires', 			value = "upgrade"},
				{label = ' Retirer Accessoires', 			value = "rupgrade"},
				{label = 'Cosmetiques', 			value = "cosmetiques"}
			}
		
			ESX.UI.Menu.Open(
			'default', GetCurrentResourceName(), 'weapon_interaction',
			{
				title    = _U('weapon_interaction'),
				align    = 'top-left',
				elements = elements
			}, function(data2, menu2)
				if not IsPedInAnyVehicle(GetPlayerPed(-1), false) and GetSelectedPedWeapon(GetPlayerPed(-1)) ~= GetHashKey("WEAPON_UNARMED") then
					...
					if action == "upgrade" then
        				OpenComponentsMenu()
        			elseif action == "rupgrade" then
        				OpenRComponentsMenu()
        			elseif action == "cosmetiques" then
        				OpenWeaponsSkinsMenu()
    				end
				else
					ESX.ShowNotification('~r~Sortez votre arme !')
				end
			end, function(data2, menu2)
				menu2.close()
			end)
		...
		if data.current.value == 'citizen_interaction' then
			...
			'default', GetCurrentResourceName(), 'citizen_interaction',
			{
				...
				if closestPlayer ~= -1 and closestDistance <= 3.0 then
					...
					--	TriggerServerEvent('ktr_iaa:message', GetPlayerServerId(closestPlayer), _U('being_searched'))
						OpenBodySearchMenu(closestPlayer)
					elseif action == 'handcuff' then
						TriggerServerEvent('ktr_iaa:handcuff', GetPlayerServerId(closestPlayer))
					elseif action == 'drag' then
						TriggerServerEvent('ktr_iaa:drag', GetPlayerServerId(closestPlayer))
					elseif action == 'put_in_vehicle' then
						TriggerServerEvent('ktr_iaa:putInVehicle', GetPlayerServerId(closestPlayer))
					elseif action == 'out_the_vehicle' then
						TriggerServerEvent('ktr_iaa:OutVehicle', GetPlayerServerId(closestPlayer))
	...
	ESX.TriggerServerCallback('ktr_iaa:getOtherPlayerData', function(data)
		...
		function(data, menu)
			...
			if data.current.value ~= nil then
				TriggerServerEvent('ktr_iaa:confiscatePlayerItem', GetPlayerServerId(player), itemType, itemName, amount)
...
function OpenFineCategoryMenu(player, category)
	...
	ESX.TriggerServerCallback('ktr_iaa:getFineList', function(fines)
...
function LookupVehicle()
	ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'lookup_vehicle',
	{
		...
		if data.value == nil or length < 2 or length > 13 then
			...
			ESX.TriggerServerCallback('ktr_iaa:getVehicleFromPlate', function(owner, found)
				if found then
...
function ShowPlayerLicense(player)
	...
	ESX.TriggerServerCallback('ktr_iaa:getOtherPlayerData', function(data)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'manage_license',
		{
			...
			TriggerServerEvent('ktr_iaa:message', GetPlayerServerId(player), _U('license_revoked', data.current.label))
...
function OpenVehicleInfosMenu(vehicleData)
	...
	ESX.TriggerServerCallback('ktr_iaa:getVehicleInfos', function(retrivedInfo)
...
function OpenGetWeaponMenu()
	...
	ESX.TriggerServerCallback('ktr_iaa:getArmoryWeapons', function(weapons)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'armory_get_weapon',
		{
			...
			ESX.TriggerServerCallback('ktr_iaa:removeArmoryWeapon', function()
				OpenGetWeaponMenu()
...
function OpenPutWeaponMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'armory_put_weapon',
	{
		...
		ESX.TriggerServerCallback('ktr_iaa:addArmoryWeapon', function()
			OpenPutWeaponMenu()
...
function OpenBuyWeaponsMenu()
	...
	for k,v in ipairs(Config.AuthorizedWeapons[PlayerData.second_job.grade_name]) do
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'armory_buy_weapons', {
		...
		if data.current.hasWeapon then
			...
			ESX.TriggerServerCallback('ktr_iaa:buyWeapon', function(bought)
				if bought then
...
function OpenGetStocksMenu()
	...
	ESX.TriggerServerCallback('ktr_iaa:getStockItems', function(items)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu',
		{
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_get_item_count', {
				...
				if count == nil then
					...
					menu2.close()
					menu.close()
					TriggerServerEvent('ktr_iaa:getStockItem', itemName, count)
...
function OpenPutStocksMenu()
	...
	ESX.TriggerServerCallback('ktr_iaa:getPlayerInventory', function(inventory)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu',
		{
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_put_item_count', {
				...
				if count == nil then
					...
					menu2.close()
					menu.close()
					TriggerServerEvent('ktr_iaa:putStockItems', itemName, count)
...
AddEventHandler('esx:setSecondJob', function(job)
  ...
  PlayerData.second_job = job
	...
	Citizen.Wait(5000)
	--TriggerServerEvent('ktr_iaa:forceBlip')
end)
...
AddEventHandler('esx_phone:cancelMessage', function(dispatchNumber)
	if type(PlayerData.second_job.name) == 'string' and PlayerData.second_job.name == 'iaa' and PlayerData.second_job.name == dispatchNumber then
...
AddEventHandler('ktr_iaa:hasEnteredMarker', function(station, part, partNum)
...
AddEventHandler('ktr_iaa:hasExitedMarker', function(station, part, partNum)
...
AddEventHandler('ktr_iaa:hasEnteredEntityZone', function(entity)
	local playerPed = PlayerPedId()

	if PlayerData.second_job ~= nil and PlayerData.second_job.name == 'iaa' and IsPedOnFoot(playerPed) then
...
AddEventHandler('ktr_iaa:hasExitedEntityZone', function(entity)
	if CurrentAction == 'remove_entity' then
		CurrentAction = nil
	end
end)

RegisterNetEvent('ktr_iaa:handcuff')
AddEventHandler('ktr_iaa:handcuff', function()
...
RegisterNetEvent('ktr_iaa:unrestrain')
AddEventHandler('ktr_iaa:unrestrain', function()
	if IsHandcuffed then
		local playerPed = PlayerPedId()
		IsHandcuffed = false
...
RegisterNetEvent('ktr_iaa:drag')
AddEventHandler('ktr_iaa:drag', function(copID)
...
RegisterNetEvent('ktr_iaa:putInVehicle')
AddEventHandler('ktr_iaa:putInVehicle', function()
	local playerPed = PlayerPedId()
	local coords = GetEntityCoords(playerPed)
...
RegisterNetEvent('ktr_iaa:OutVehicle')
AddEventHandler('ktr_iaa:OutVehicle', function()
	local playerPed = PlayerPedId()
...
Citizen.CreateThread(function()
	while true do
		...
		if PlayerData.second_job ~= nil and PlayerData.second_job.name == 'iaa' then
			...
			for k,v in pairs(Config.PoliceStations) do
				...
				if Config.EnablePlayerManagement and PlayerData.second_job.grade_name == 'boss' or PlayerData.second_job.grade_name == 'chef' then
			...
			if isInMarker and not HasAlreadyEnteredMarker or (isInMarker and (LastStation ~= currentStation or LastPart ~= currentPart or LastPartNum ~= currentPartNum)) then
				...
				then
					TriggerEvent('ktr_iaa:hasExitedMarker', LastStation, LastPart, LastPartNum)
					hasExited = true
				end
				...
				TriggerEvent('ktr_iaa:hasEnteredMarker', currentStation, currentPart, currentPartNum)
	...
	while true do
		...
		if closestDistance ~= -1 and closestDistance <= 3.0 then
			if LastEntity ~= closestEntity then
				TriggerEvent('ktr_iaa:hasEnteredEntityZone', closestEntity)
				LastEntity = closestEntity
			end
		else
			if LastEntity ~= nil then
				TriggerEvent('ktr_iaa:hasExitedEntityZone', LastEntity)
				LastEntity = nil
			end
		end
	end
...
Citizen.CreateThread(function()
	while true do
		...
		if CurrentAction ~= nil then
			...
			if IsControlJustReleased(0, Keys['E']) and PlayerData.second_job ~= nil and PlayerData.second_job.name == 'iaa' then
		...
		if IsControlJustReleased(0, Keys['F2']) and not isDead and PlayerData.second_job ~= nil and PlayerData.second_job.name == 'iaa' and not ESX.UI.Menu.IsOpen('default', GetCurrentResourceName(), 'police_actions') then
			if Config.MaxInService == -1 then
...
function createBlip(id)
	...
	if not DoesBlipExist(blip) then -- Add blip and create head display on player
		...
		table.insert(blipsFIB, blip) -- add blip to array so we can remove it later
	end
end

RegisterNetEvent('ktr_iaa:updateBlip')
AddEventHandler('ktr_iaa:updateBlip', function()
	
	-- Refresh all blips
	for k, existingBlip in pairs(blipsFIB) do
	...
	if PlayerData.second_job ~= nil and PlayerData.second_job.name == 'iaa' then
		ESX.TriggerServerCallback('esx_society:getOnlinePlayers', function(players)
			for i=1, #players, 1 do
				if players[i].second_job.name == 'iaa' then
...
AddEventHandler('playerSpawned', function(spawn)
	isDead = false
	TriggerEvent('ktr_iaa:unrestrain')
	
	if not hasAlreadyJoined then
		TriggerServerEvent('ktr_iaa:spawned')
	end
...
AddEventHandler('onResourceStop', function(resource)
	if resource == GetCurrentResourceName() then
		TriggerEvent('ktr_iaa:unrestrain')
		TriggerEvent('esx_phone:removeSpecialContact', 'iaa')
	...
	ESX.UI.Menu.Open(
        ...
        function(data, menu)
        	if data.current.value == "silent" then
        		TriggerEvent("esx_weaponsaccessories:equipSilent")
        	elseif data.current.value == "flashlight" then
        		TriggerEvent("esx_weaponsaccessories:equipFlashlight")
    		elseif data.current.value == "grip" then
        		TriggerEvent("esx_weaponsaccessories:equipGrip")
        	elseif data.current.value == "emag" then
        		TriggerEvent("esx_weaponsaccessories:equipEmag")
        	elseif data.current.value == "vemag" then
        		TriggerEvent("esx_weaponsaccessories:equipVEmag")
        	elseif data.current.value == "scope" then
        		TriggerEvent("esx_weaponsaccessories:equipScope")
        	elseif data.current.value == "ascope" then
        		TriggerEvent("esx_weaponsaccessories:equipAdvancedcope")
        	elseif data.current.value == "yusuf" then
        		TriggerEvent("esx_weaponsaccessories:equipYusuf")
        	elseif data.current.value == "lowrider" then
        		TriggerEvent("esx_weaponsaccessories:equipLowrider")
        	elseif data.current.value == "incendiary" then
        		TriggerEvent("esx_weaponsaccessories:equipIncendiary")
        	elseif data.current.value == "tracer" then
        		TriggerEvent("esx_weaponsaccessories:equipTracer")
        	elseif data.current.value == "lazer_scope" then
        		TriggerEvent("esx_weaponsaccessories:equipLazerScope")
        	elseif data.current.value == "compansator" then
        		TriggerEvent("esx_weaponsaccessories:equipCompansator")
        	elseif data.current.value == "thermal_scope" then
        		TriggerEvent("esx_weaponsaccessories:equipThermalScope")
        	elseif data.current.value == "nightvision_scope" then
        		TriggerEvent("esx_weaponsaccessories:equipNightvisionScope")
        	elseif data.current.value == "barrel" then
        		TriggerEvent("esx_weaponsaccessories:equipBarrel")
    		elseif data.current.value == "fmj" then
        		TriggerEvent("esx_weaponsaccessories:equipFMJ")
        	elseif data.current.value == "hollow" then
        		TriggerEvent("esx_weaponsaccessories:equipHollow")
        	end
        end,
        function(data, menu)
            menu.close()
...
function openCanon()
	...
	TriggerEvent('NB:openMenuIAACanon')
end