...
function StarteventJob()
  ...
  ShowLoadingPromt(_U('taking_service') .. 'Uber', 5000, 3)
  ClearCurrentMission()
      ...
      if data.current.value == 'spawn_vehicle' then
        ...
        if Config.EnableSocietyOwnedVehicles then
          ...
          if Config.MaxInService == -1 then
            ...
            ESX.Game.SpawnVehicle('baller3', coords, 170.0, function(vehicle)
              TaskWarpPedIntoVehicle(playerPed,  vehicle, -1)
            end)
...
AddEventHandler('ktr_luxuryuber:hasEnteredMarker', function(zone)
...
AddEventHandler('ktr_luxuryuber:hasExitedMarker', function(zone)
  ESX.UI.Menu.CloseAll()
  CurrentAction = nil
end)
  ...
  while true do
    ...
    if PlayerData.job ~= nil and PlayerData.job.name == 'event' then
      ...
      if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
        ...
        TriggerEvent('ktr_luxuryuber:hasEnteredMarker', currentZone)
      end

      if not isInMarker and HasAlreadyEnteredMarker then
        HasAlreadyEnteredMarker = false
        TriggerEvent('ktr_luxuryuber:hasExitedMarker', LastZone)
      end
          ...
          if IsPedSittingInVehicle(CurrentCustomer,  vehicle) then
            ...
            if CustomerEnteredVehicle then
              ...
              if targetDistance <= 10.0 then
                ...
                TriggerServerEvent('ktr_luxuryuber:success')
...
Citizen.CreateThread(function()
  while true do
      ...
      if IsControlPressed(0,  Keys['E']) and PlayerData.job ~= nil and PlayerData.job.name == 'event' and (GetGameTimer() - GUI.Time) > 300 and GetLastInputMethod(2) then
        ...
        if CurrentAction == 'delete_vehicle' then
          ...
          if Config.EnableSocietyOwnedVehicles then
            ...
            if GetEntityModel(CurrentActionData.vehicle) == GetHashKey('baller3') then
              if Config.MaxInService ~= -1 then
          ...
          function(data, menu)
            ...
            if amount == nil or amount < 0 then
              ...
              if closestPlayer == -1 or closestDistance > 3.0 then
                ...
                TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(closestPlayer), 'society_event', ('Luxurt Event'), amount)
           --  end)
							end
						end
					end,
   ...
   if IsControlJustReleased(0, Keys['F6']) and not isDead and PlayerData.job ~= nil and PlayerData.job.name == 'event' and not ESX.UI.Menu.IsOpen('default', GetCurrentResourceName(), 'event_actions') then
				OpeneventActionsMenu2()
	end
--end
--end)
...
RegisterNetEvent('ktr_luxuryuber:annonce')
AddEventHandler('ktr_luxuryuber:annonce', function(text)
 		texteafiche = text
 		affichenews = true
...
RegisterNetEvent('ktr_luxuryuber:annoncestop')
AddEventHandler('ktr_luxuryuber:annoncestop', function()
 		affichenews = false
