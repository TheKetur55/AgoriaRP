...
RegisterNetEvent('NB:openMenuBesoin')
AddEventHandler('NB:openMenuBesoin', function()
        ...
        if Config.StatusBars then
            TriggerEvent('esx_status:getStatus', 'pee', function(status)
                if status.val >= 1000000 then
                    ...
                    TriggerClientEvent('esx_basicneeds:peefait')
        ...
        if Config.StatusBars then
            TriggerEvent('esx_status:getStatus', 'poo', function(status)
                if status.val < 200000 then
                    ...
                    TriggerClientEvent('esx_basicneeds:poofait')
...
function OpenNeedsMenu()
    ESX.UI.Menu.Open(
        ...
        {
            title    = 'Besoin Pipi / Caca',
            align    = 'top-left',
            elements = { 
                { label = 'Faire pipi', value = 'pee' },
                { label = 'Faire caca', value = 'poop' }
            }
        },
    function(data, menu)
