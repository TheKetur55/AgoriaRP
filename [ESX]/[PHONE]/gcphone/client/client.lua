...
Citizen.CreateThread(function ()
  ...
  while true do 
    ...
    if inRangeToActivePhone == true and currentPlaySound == false  then --55
    --	TriggerEvent('esx:showNotification', '~r~Vous avez un appel')
...
AddEventHandler("gcPhone:receiveMessage", function(message)
  ...
  if message.owner == 0 and onDuty then --4555
...
AddEventHandler("gcPhone:waitingCall", function(infoCall, initiator)
  ...
  if initiator == true and onDuty then
    PhonePlayCall()
    if menuIsOpen == false then
      TooglePhone()
    end
  ...
  if data.eventName ~= 'cancel' then
    if data.data ~= nil then 
      ...
      if data.eventName == "police" then
        TriggerEvent('esx:showNotification', '~h~Vous avez appelé la ~b~Police')
	  elseif data.eventName == "event" then
        TriggerEvent('esx:showNotification', '~h~Vous avez appelé le ~b~Uber')
      elseif data.eventName == "mecano" then
        TriggerEvent('esx:showNotification', '~h~Vous avez appelé les ~b~Mécano')
      elseif data.eventName == "ambulance" then
        TriggerEvent('esx:showNotification', '~h~Vous avez appelé les ~b~Ambulancier')
      end
...
RegisterNUICallback('faketakePhoto', function(data, cb)
  ...
  TriggerEvent('esx:showNotification', '~r~Pressez F1 pour fermer')
	TriggerEvent('esx:showNotification', '~g~Pressez G pour changer de mode')
end)
...
RegisterNUICallback('takePhoto', function(data, cb)
	TriggerEvent('esx:showNotification', '~r~Pressez F1 pour fermer')
	TriggerEvent('esx:showNotification', '~g~Pressez G pour changer de mode')
	TriggerEvent('esx:showNotification', '~b~Pressez E pour prendre une photo [SOON]')
	CreateMobilePhone(1)
  ...
  if hasFocus == true then
    ...
    Citizen.Wait(0)
		...
		if IsControlJustPressed(1, 47) then -- Toogle Mode
			frontCam = not frontCam
			CellFrontCamActivate(frontCam)
    elseif IsControlJustPressed(1, 288) then -- CANCEL
      DestroyMobilePhone()
