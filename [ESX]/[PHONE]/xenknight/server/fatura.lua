...
function FaturaGetBilling (accountId, cb)
  local xPlayer = ESX.GetPlayerFromId(source)
    MySQL.Async.fetchAll([===[
      SELECT * FROM billing WHERE identifieri = @identifieri
      ]===], { ['@identifieri'] = xPlayer.identifier }, cb)
  end 
    ...
    if amount ~= nil then
        if xPlayer.getBank() >= amount then	
					...
					MySQL.Async.execute('DELETE from billing WHERE id = @id', {
						...
						TriggerClientEvent("esx:showNotification", sender, "Vous ne pouvez pas vous permettre de payer cette facture.")
