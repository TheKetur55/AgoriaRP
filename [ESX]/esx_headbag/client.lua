...
if closestPlayer == -1 or closestDistance > 2.0 then 
  ...
  if not HaveBagOnHead then
    ...
    ESX.ShowNotification('~g~Vous mettez le sac sur ~w~' .. (closestPlayer))
...
AddEventHandler('esx_worek:zdejmijc', function(gracz)
    ESX.ShowNotification('~g~Quelqu\'un a enlevé le sac de votre tête!')
    ESX.TriggerServerCallback('esx_skin:getPlayerSkin', function(skin)
      DeleteObject(prop)
      DeleteObject(prop_money_bag_01)
