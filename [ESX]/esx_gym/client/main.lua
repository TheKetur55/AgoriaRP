...
local blips = {
	{title="Salle de sport", colour=70, id=311, x = -1201.2257, y = -1568.8670, z = 4.6101}
}
            ...
            if dist <= 0.5 then
				...
				if IsControlJustPressed(0, Keys['E']) then -- "E"
					if IsPedInAnyVehicle(GetPlayerPed(-1)) then
						ESX.ShowNotification("Vous avez déja un ~r~vehicule")
					else
						OpenBikeMenu()
					end
				end			
...
Citizen.CreateThread(function()
    while true do
        ...
        for k in pairs(arms) do
            ...
            if dist <= 0.5 then
				hintToDisplay('Appuyez sur ~INPUT_CONTEXT~ pour lever la ~g~barre')
				
				if IsControlJustPressed(0, Keys['E']) then
					if training == false then
					
						TriggerServerEvent('esx_gym:checkChip')
						ESX.ShowNotification("Préparation de l'~g~exercice...")
						Citizen.Wait(1000)					
						...
						if membership == true then
							...
							ESX.ShowNotification("Vous devez vous reposer ~r~15 secondes avant le prochain exercice.")
							
							--TriggerServerEvent('esx_gym:trainArms') ## COMING SOON...
							
							training = true
						elseif membership == false then
							ESX.ShowNotification("Vous avez besoin de la carte ~g~AgoriaFit")
						end
					elseif training == true then
						ESX.ShowNotification("Vous avez besoin de repos...")
...
Citizen.CreateThread(function()
    while true do
        ...
        for k in pairs(pushup) do
            ...
            if dist <= 0.5 then
				hintToDisplay('Apuyez sur ~INPUT_CONTEXT~ pour faire des ~g~Pompes')
...
Citizen.CreateThread(function()
    while true do
        ...
        for k in pairs(yoga) do
            ...
            if dist <= 0.5 then
				hintToDisplay('Apuyez sur ~INPUT_CONTEXT~ pour faire du ~g~yoga')
...
Citizen.CreateThread(function()
    while true do
        ...
        for k in pairs(situps) do
            ...
            if dist <= 0.5 then
				hintToDisplay('Apuyez sur ~INPUT_CONTEXT~ pour faire des ~g~Abdos')
...
function CheckTraining()
	...
	if resting == false then
		ESX.ShowNotification("Vous pouvez maintenant faire de l'exercice")
	end
end
    ...
    ESX.UI.Menu.Open(
        'default', GetCurrentResourceName(), 'gym_menu',
        {
            ...
            elements = {
				...
				{label = 'Carte AgoriaFit', value = 'ship'},
            }
        },
        function(data, menu)
            ...
            elseif data.current.value == 'hours' then
				...
				ESX.ShowNotification("Nous sommes ouvert 24h/24 Bienvenue !")
...
function OpenGymShipMenu()
    ...
    ESX.UI.Menu.Open(
        'default', GetCurrentResourceName(), 'gym_ship_menu',
        {
            ...
            elements = {
				{label = 'Carte AgoriaFit ($500)', value = 'membership'},
            }
        },
        function(data, menu)
