...
AddEventHandler('esx_vehiclelock:givekeycardealer', function(plate, target)
local toplate = plate
...
MySQL.Async.execute(
		'INSERT INTO open_car (label, value, NB, got, identifier) VALUES (@label, @value, @NB, @got, @identifier)',
		{
			...
			['@NB']   		  = 1,
			['@got']  		  = 'true',
			['@identifier']   = xPlayertarget
		}
,		function(result)
			TriggerClientEvent('esx:showNotification', xPlayertarget, '~g~Vous avez reçu les clés de votre véhicule ~g~')
		end)
end)
...
RegisterServerEvent('esx_vehiclelock:dvktr')
AddEventHandler('esx_vehiclelock:dvktr', function(plate, target)
local toplate = plate
local xPlayertarget = target
print(toplate)
print(xPlayertarget)
MySQL.Async.fetchAll(
		...
		function(result)
			...
			TriggerClientEvent('esx:showNotification', xPlayertarget, '~g~Vous avez rendue vos clé ~g~')
			for i=1, #result, 1 do
			MySQL.Async.execute(
			'DELETE FROM open_car WHERE id = @id',
			{
...
MySQL.Async.execute(
		...
		function(result)
				TriggerClientEvent('esx:showNotification', _source, 'Vous avez reçu les clés.')
		end)
end)
-----------------------------------------------------------------------------------------
-----------------------------------------------------------------------------------------
...
MySQL.Async.execute(
		...
		function(result)
				TriggerClientEvent('esx:showNotification', xPlayer.source, 'Vous avez reçu de nouvelles clés.')
				TriggerClientEvent('esx:showNotification', _source, 'Vous avez reçu les clés.')
		end)
...
MySQL.Async.execute(
		...
		function(result)
				TriggerClientEvent('esx:showNotification', xPlayertarget.source, 'Vous avez reçu de nouvelles clés')
				TriggerClientEvent('esx:showNotification', xPlayer.source, 'Vous avez donné vos clés.')
		end)
end)
...
MySQL.Async.execute(
		...
		function(result)
				...
				TriggerClientEvent('esx:showNotification', xPlayer.source, 'Vous avez prété votre double des clés.')
		end)
