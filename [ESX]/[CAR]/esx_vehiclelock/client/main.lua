		...
		for i=1, #mykey, 1 do
			if mykey[i].got == 'true' then 
				if 	mykey[i].NB == 1 then									
						table.insert(elements, {label = 'Clés du véhicule immatriculé : '.. ' [' .. mykey[i].plate .. ']', value = mykey[i].plate})
					elseif mykey[i].NB == 2 then
						table.insert(elements, {label = 'Double du véhicule immatriculé : '.. ' [' .. mykey[i].plate .. ']', value = nil})
					end
				end
			end
	...
	'default', GetCurrentResourceName(), 'mykey',
	{
        ...
        ESX.UI.Menu.CloseAll()
  			ESX.UI.Menu.Open(
				...
				align = 'top-left',
				elements = {
						{label = 'Donner les clés', value = 'donnerkey'}, -- Donné les clés
						{label = 'Préter les clés', value = 'preterkey'}, -- Donné les clés
			  		},
	  			},
	...
	'default', GetCurrentResourceName(), 'mykey',
	{
        ...
        ESX.UI.Menu.CloseAll()
  			ESX.UI.Menu.Open(
				...
				align = 'top-left',
				elements = {
						{label = 'Préter les clés', value = data2.current.value}, -- Donné les clés
			  		},
	  			},
        		function(data3, menu3) --Submit Cb
 					local player, distance = ESX.Game.GetClosestPlayer()
 ...
 RequestAnimDict(dict)
    while not HasAnimDictLoaded(dict) do
		...
		if gotkey then
			...
			elseif locked == 2 or locked == 3 or locked == 4 then -- if locked
		...
		for i=1, #mykey, 1 do
			if mykey[i].got == 'true' then 
				if 	mykey[i].NB == 3 then									
						table.insert(elements, {label = 'Clés du véhicule de société : '.. ' [' .. mykey[i].plate .. ']', value = mykey[i].plate})
					end
				end
			end
	...
	'default', GetCurrentResourceName(), 'mykey',
	{
        ...
        ESX.UI.Menu.CloseAll()
  			ESX.UI.Menu.Open(
				...
				title = 'Voulez-vous ?',
				align = 'top-left',
				elements = {{label = 'Donner les clés', value = data2.current.value}, -- Donné un double
			  		},
	  			},
        		function(data3, menu3) --Submit Cb
 					local player, distance = ESX.Game.GetClosestPlayer()
        ...
        if data.current.value == 'registerkey' then
					ESX.TriggerServerCallback('esx_vehiclelock:getVehiclesnokey', function(Vehicles2)
						...
						if Vehicles2 == nil then
							table.insert(elements, {label = 'Vous avez les clés de tout vos véhicules', value = nil})
						else
							...
							{
							title    = 'Les clés sécurisées coûtent 300$.',
							align    = 'top-left',
							elements = elements
							},
...
Citizen.CreateThread(function()
  while true do
    ...
    if CurrentAction ~= nil then
      ...
      AddTextComponentString('Appuyez sur ~INPUT_CONTEXT~')
