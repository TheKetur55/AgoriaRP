...
function OpenMenuFourriere()
	...
	ESX.UI.Menu.Open(
		'default', GetCurrentResourceName(), 'fourriere_menu',
		{
			css 	 = 'four',
			title    = 'Fourrière',
			align    = 'top-left',
...
AddEventHandler('garage:hasEnteredMarker', function(zone)
	if zone == 'garage' then
		CurrentAction     = 'fourriere_action_menu'
		CurrentActionMsg  = "Appuyer sur ~INPUT_PICKUP~ pour ouvrir la ~y~Fourrière~s~"
		CurrentActionData = {}
	end
end)
...
Citizen.CreateThread(function()
	while true do
		...
		if CurrentAction ~= nil then
			...
			if IsControlPressed(0,  Keys['E']) and (GetGameTimer() - GUI.Time) > 150 and GetLastInputMethod(2) then

				if CurrentAction == 'fourriere_action_menu' then
					OpenMenuFourriere()
				end
