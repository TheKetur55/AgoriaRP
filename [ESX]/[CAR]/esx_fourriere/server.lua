...
AddEventHandler('garage:modifystate', function(vehicleProps, state)
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
	local vehicules = getPlayerVehicles(xPlayer.getIdentifier())
	local plate = vehicleProps.plate
...
ESX.RegisterServerCallback('garage:getOutVehicles',function(source, cb)	
	...
	MySQL.Async.fetchAll("SELECT * FROM owned_vehicles3 WHERE owner=@identifier AND `stored`=false",{['@identifier'] = xPlayer.getIdentifier()}, function(data) 
		for _,v in pairs(data) do
			local vehicle = json.decode(v.vehicle)
...
AddEventHandler('garage:pay', function()
	...
	TriggerClientEvent('esx:showNotification', source, 'Vous avez payé ' .. Config.Price .. ' ~g~$~s~')
