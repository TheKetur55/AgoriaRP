...
ESX.RegisterServerCallback("esx-qalle-sellvehicles:buyVehicle", function(source, cb, vehProps, price)
	...
	if xPlayer.getAccount("bank")["money"] >= price or price == 0 then
		...
		TriggerEvent('esx_vehiclelock:givekeycardealer', plate, xPlayer.identifier)
...
function UpdateCash(identifier, cash)
	...
	if xPlayer ~= nil then
		...
		TriggerClientEvent("esx:showNotification", xPlayer.source, "Véhicule envoyer a la fourriere $" .. cash)
	else
