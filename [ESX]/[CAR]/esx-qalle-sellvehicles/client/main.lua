...
--[[
		...
		AddTextComponentString("Agoria SecondChance")
		...
		while true do
			...
			if dstCheck <= 10.0 then
				...
				if dstCheck <= 4.2 then
					ESX.Game.Utils.DrawText3D(SellPos, "[E] Vendre un véhicule", 0.4)
					if IsControlJustPressed(0, 38) then
						if IsPedInAnyVehicle(ped, false) then
							OpenSellMenu(GetVehiclePedIsUsing(ped))
						else
							ESX.ShowNotification("Vous devez etre dans un ~g~vehicule")
						end
					end
				end
			end
					...
					if dstCheck <= 2.0 then
						...
						if IsControlJustPressed(0, 38) then
							if IsPedInVehicle(ped, Config.VehiclePositions[i]["entityId"], false) then
								...
								ESX.ShowNotification("Vous devez etre dans un ~g~vehicule~s~!")
							end
						end
					end
				end
...
function OpenSellMenu(veh, price, buyVehicle, owner)
	...
	if not buyVehicle then
		...
		if owner then
			table.insert(elements, { ["label"] = "Retirer le véhicule", ["value"] = "remove" })
		end
	end

	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'sell_veh',
		{
			title    = "Agoria SecondChance",
			align    = 'top-right',
			elements = elements
		},
	function(data, menu)
		...
		elseif action == "sell" then
			...
			ESX.TriggerServerCallback("esx-qalle-sellvehicles:isVehicleValid", function(valid)
				...
				if valid then
					DeleteVehicle(veh)
					ESX.ShowNotification("Vous mettez le ~g~véhicule ~s~ pour - " .. price .. "$")
					menu.close()
				else
					ESX.ShowNotification("Ce n'est pas votre ~r~véhicule!~s~")
				end
		...
		elseif action == "buy" then
			ESX.TriggerServerCallback("esx-qalle-sellvehicles:buyVehicle", function(isPurchasable, totalMoney)
				if isPurchasable then
					DeleteVehicle(veh)
					ESX.ShowNotification("~r~Véhicule a la fourriere")
					ESX.ShowNotification("Vous avez ~g~acheté~s~ le Véhicule pour " .. price .. " :-")
					menu.close()
				else
					ESX.ShowNotification("~r~Vous n'avez pas assez d'argent, il vous manque " .. price - totalMoney .. "$ en banque")
				end
		...
		elseif action == "remove" then
			ESX.TriggerServerCallback("esx-qalle-sellvehicles:buyVehicle", function(isPurchasable, totalMoney)
				if isPurchasable then
					ESX.ShowNotification("~r~Véhicule a la fourriere")
					DeleteVehicle(veh)
					ESX.ShowNotification("Vous avez retirer votre véhicule")
					menu.close()
				end
