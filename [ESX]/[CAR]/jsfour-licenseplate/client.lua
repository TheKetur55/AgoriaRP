...
AddEventHandler('jsfour-licenseplate', function()
  ...
  ESX.UI.Menu.Open(
    ...
    align    = 'bottom-right',
		elements = {
			{label = 'Changer la plaque', value = 'change'},
			{label = 'Appliquer à la voiture', value = 'apply'},
		}
	},
	function(data, menu)
        ...
        if distance ~= -1 and distance <= 3.0 then
          ...
          ESX.TriggerServerCallback('jsfour-licenseplate:update', function( cb )
            ...
            elseif cb == 'error' then
              ESX.ShowNotification('Vous ne pouvez pas utilisercette plaque !')
            end
          end, oldplate, plate)
        else
          ESX.ShowNotification('Aucun véhicule devant vous !')
        end
      else
        ESX.ShowNotification('Impossible d\'appliquer une plaque vide !')
      end
		end
	end,
	function(data, menu)
