...
function ListVehiclesMenu(garage, KindOfVehicle)
	local elements = {}
	local vehicleName = ""
	ESX.TriggerServerCallback('eden_garage2:getVehicles', function(vehicles)
		if not table.empty(vehicles) then
			...
			table.insert(elements, {label ="Pas de bateau dans le garage" , value = nil})
		end
		ESX.UI.Menu.Open(
		...
		function(data, menu)
			local elem = {}
			table.insert(elem, {label ="Sortir le bateau" , value = 'get_vehicle_out'})
			table.insert(elem, {label ="Renommer le bateau" , value = 'rename_vehicle'})
				...
				function(data2, menu2)
					...
					elseif data2.current.value == "rename_vehicle" then
						...
						if (GetOnscreenKeyboardResult()) then
							...
							TriggerServerEvent('eden_garage2:renamevehicle', data.current.value.plate, name)
						end
					end
				end,
				function(data2, menu2)
...
function ListVehiclesFourriereMenu(garage)
	...
	ESX.TriggerServerCallback('eden_garage2:getVehiclesMecano', function(vehicles)
		...
		function(data, menu)
			...
			TriggerServerEvent('eden_garage2:ChangeStateFromFourriereMecano', data.current.value.vehicle, false)
		end,
		function(data, menu)
			menu.close()
		end
	...
	if IsPedInAnyVehicle(playerPed,  false) then
		...
		if GetPedInVehicleSeat(vehicle, -1) == playerPed then
			...
			if GotTrailer then
				...
				ESX.TriggerServerCallback('eden_garage2:stockv',function(valid)
					if(valid) then
						...
						TriggerServerEvent('eden_garage2:modifystate', trailerProps.plate, true)
				...
				ESX.TriggerServerCallback('eden_garage2:stockv',function(valid)
					if(valid) then
						...
						TriggerServerEvent('eden_garage2:modifystate', vehicleProps.plate, true)
...
function StockVehicleFourriereMenu()
	...
	if IsPedInAnyVehicle(playerPed,  false) then
		...
		if GetPedInVehicleSeat(vehicle, -1) == playerPed then
			...
			if GotTrailer then
				local trailerProps  = ESX.Game.GetVehicleProperties(TrailerHandle)
				ESX.TriggerServerCallback('eden_garage2:stockvmecano',function(valid)
					if(valid) then
...
function SpawnVehicle(vehicle, garage, KindOfVehicle)
	...
	TriggerServerEvent('eden_garage2:modifystate', vehicle.plate, false)
end
...
function SpawnVehicleMecano(vehicle, garage)
	...
	TriggerServerEvent('eden_garage2:ChangeStateFromFourriereMecano', vehicle, false)
end
--Fin fonction pour spawn vehicule fourriere mecano

function ReturnVehicleMenu(garage, KindOfVehicle)

	ESX.TriggerServerCallback('eden_garage2:getOutVehicles', function(vehicles)
		local elements = {}
		if not table.empty(vehicles) then
			for _,v in pairs(vehicles) do
				v.vehicle = json.decode(v.vehicle)
		...
		function(data, menu)
			...
			elseif data.current.value ~= nil then
				...
				if not iscaronearth then
					ESX.TriggerServerCallback('eden_garage2:checkMoney', function(hasEnoughMoney)
...
AddEventHandler('ft_libs:OnClientReady', function()
	for k,v in pairs (Config.Garages) do
		this_Garage = v
		exports.ft_libs:AddArea("esx_eden_garage2_area_"..k.."_garage", {
			marker = {
				weight = v.Marker.w,
				height = v.Marker.h,
				red = v.Marker.r,
		...
		exports.ft_libs:AddArea("esx_eden_garage2_area_"..k.."_spawnpoint", {
			marker = {
		...
		exports.ft_libs:AddArea("esx_eden_garage2_area_"..k.."_deletepoint", {
			marker = {
	...
	for k,v in pairs (Config.GaragesMecano) do
		exports.ft_libs:AddArea("esx_eden_garage2_area_"..k.."_mecanospawnpoint", {
			enable = false,
			marker = {
				weight = v.SpawnPoint.Marker.w,
				height = v.SpawnPoint.Marker.h,
		...
		exports.ft_libs:AddArea("esx_eden_garage2_area_"..k.."_mecanodeletepoint", {
			enable = false,
			marker = {
				weight = v.DeletePoint.Marker.w,
				height = v.DeletePoint.Marker.h,
...
RegisterNetEvent('esx_eden_garage2:ListVehiclesMenu')
AddEventHandler('esx_eden_garage2:ListVehiclesMenu', function(garage, society)
	ListVehiclesMenu(garage, society)
end)

RegisterNetEvent('esx_eden_garage2:OpenMenuGarage')
AddEventHandler('esx_eden_garage2:OpenMenuGarage', function(garage, society)
	OpenMenuGarage(garage, society)
end)

RegisterNetEvent('esx_eden_garage2:StockVehicleMenu')
AddEventHandler('esx_eden_garage2:StockVehicleMenu', function(society)
