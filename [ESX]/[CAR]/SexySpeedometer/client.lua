...
Citizen.CreateThread(function()
	TriggerEvent('chat:addSuggestion', '/hud', 'Activer/Désactiver le Speedometer' )
	...
	while true do
		...
		if not HasStreamedTextureDictLoaded(cst.ytdName) then
			...
			if showFuelGauge and FuelLevel and MaxFuelLevel ~= 0 then
				...
				DrawSprite(cst.ytdName, cst.FuelNeedle or curNeedle, cst.centerCoords[1]+cst.FuelGaugeLoc[1],cst.centerCoords[2]+cst.FuelGaugeLoc[2],cst.FuelGaugeLoc[3],cst.FuelGaugeLoc[4],80.0+FuelLevel/MaxFuelLevel*80, 255, 255, 255, curAlpha)
			end
		end
	end
