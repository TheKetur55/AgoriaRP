...
ESX.RegisterServerCallback('eden_garage2:getOutVehicles2',function(source, cb, KindOfVehicle)	
	local _source = source
	local vehicules = {}
	local identifier = ""
	if KindOfVehicle ~= "personal" then
	...
	MySQL.Async.fetchAll("SELECT * FROM owned_vehicles3 WHERE owner=@identifier AND (`stored`=false OR fourrieremecano=true)",{['@identifier'] = identifier}, function(data) 
		for _,v in pairs(data) do
			table.insert(vehicules, {vehicle = v.vehicle, fourrieremecano = v.fourrieremecano, vehiclename =  v.vehiclename})
		end
		cb(vehicules)
