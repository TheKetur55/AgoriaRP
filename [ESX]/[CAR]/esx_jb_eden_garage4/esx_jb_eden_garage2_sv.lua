...
ESX.RegisterServerCallback('eden_garage4:getVehicles', function(source, cb, KindOfVehicle)
...
--Recupere les véhicules
ESX.RegisterServerCallback('eden_garage4:getVehiclesMecano', function(source, cb)
	local _source = source
	local vehicules = {}
...
--Stock les véhicules
ESX.RegisterServerCallback('eden_garage4:stockv',function(source,cb, vehicleProps, KindOfVehicle)
	local identifier = ""
	local _source = source
	if KindOfVehicle ~= "personal" then
		identifier = KindOfVehicle
	...
	MySQL.Async.fetchAll("SELECT * FROM owned_vehicles where plate=@plate and owner=@identifier",{['@plate'] = vehplate, ['@identifier'] = identifier}, function(result)  
		if result[1] ~= nil then
			...
			if originalvehprops.model == vehiclemodel then
				...
				print("[esx_eden_garage4] player "..identifier..' tried to spawn a vehicle with hash:'..vehiclemodel..". his original vehicle: "..originalvehprops.model)
				cb(false)
			end
		else
			cb(false)
...
ESX.RegisterServerCallback('eden_garage4:stockvmecano',function(source,cb, vehicleProps)
	local _source = source
...
RegisterServerEvent('eden_garage4:modifystate')
AddEventHandler('eden_garage4:modifystate', function(plate, state)
...
RegisterServerEvent('eden_garage4:ChangeStateFromFourriereMecano')
AddEventHandler('eden_garage4:ChangeStateFromFourriereMecano', function(vehicleProps, fourrieremecano)
	local _source = source
...
RegisterServerEvent('eden_garage4:renamevehicle')
AddEventHandler('eden_garage4:renamevehicle', function(vehicleplate, name)
...
ESX.RegisterServerCallback('eden_garage4:getOutVehicles',function(source, cb, KindOfVehicle)	
	local _source = source
	local vehicules = {}
	local identifier = ""
	if KindOfVehicle ~= "personal" then
...
RegisterServerEvent("moneyret")
AddEventHandler("moneyret", function(chargeAmount)
	local _source = source
...
ESX.RegisterServerCallback('eden_garage4:checkMoney', function(source, cb)
	local xPlayer = ESX.GetPlayerFromId(source)
