...
Citizen.CreateThread(function()
 ...
 ------------------------------------------
----------Gestion Moteur ON/OFF ------------
------------------------------------------
-- Éteindre/allumer le moteur
Citizen.CreateThread(function()
 while true do
    Citizen.Wait(1500)
	if IsControlPressed(0, Keys["L"]) and GetLastInputMethod(0) then
		  local playerPed   = PlayerPedId()
		  local vehiclePed  = GetVehiclePedIsIn(playerPed)
		  if vehiclePed and GetPedInVehicleSeat(vehiclePed, -1) == playerPed then
			local engine    = GetIsVehicleEngineRunning(vehiclePed)
				  ...
				  if engine then
					  ...
					  TriggerEvent('esx:showNotification', "Le moteur de votre voiture est ~r~Éteint", 'CHAR_PLANESITE','Statut Moteur')
				--	  ESX.ShowNotification('~r~Moteur Éteint')
				  else
					  SetVehicleEngineOn(vehiclePed, true, false, true)
					  SetVehicleUndriveable(vehiclePed, false)
					  TriggerEvent('esx:showNotification', "Le moteur de votre voiture est ~g~Allumé", 'CHAR_PLANESITE','Statut Moteur')
					--  ESX.ShowNotification('~v~Moteur Allumé')
			end
		end
...
Citizen.CreateThread(function()
 while true do
    Citizen.Wait(1000)
	if IsControlPressed(0, Keys["F"]) and GetLastInputMethod(2) then
		  local playerPed   = PlayerPedId()
		  local vehiclePed  = GetVehiclePedIsIn(playerPed)
		  if vehiclePed and GetPedInVehicleSeat(vehiclePed, -1) == playerPed then
					  ...
					  SetVehicleEngineOn(vehiclePed,  true,  true)
				--	  TriggerEvent('esx:showNotification', "Le moteur de votre voiture est ~r~Éteint", 'CHAR_PLANESITE','Statut Moteur')
  ...
  Citizen.Wait(0)
    ...
    --    local vehicleId GetVehiclePedIsUsing(GetPlayerPed(-1))
      ...
      velBuffer[1] = GetEntityVelocity(car)
 	  ...
 	  if IsControlJustPressed(1, 29)  and GetLastInputMethod(0) then       
        beltOn = not beltOn 
        if beltOn then 
		  TriggerEvent('esx_extended:showNotification', "Ceinture ~g~bouclée", 'CHAR_PLANESITE','Statut Ceinture')
		...
		--  TriggerEvent('sd:save', vehicleId)
		  TriggerEvent('esx_extended:showNotification', "Ceinture ~r~enlevée", 'CHAR_PLANESITE','Statut Ceinture')
...
---fin Fonction Ceinture de sécurité:
--Anti vole MOLDUE / NPC 
--[[
Citizen.CreateThread(function()
	while true do
		Citizen.Wait(0)
...
-- local TeleportFromTo = {
	...
	-- ["Garage Bunker"] = {
		-- positionFrom = { ['x'] =724.3556, ['y'] = -2991.792, ['z'] = -39.999, nom = "Sortir du LS CUSTOM VIP"},
		-- positionTo = { ['x'] = -74.556, ['y'] = 6423.826, ['z'] = 30.490 , nom = "Entrer du LS CUSTOM VIP"},
	-- },
	
	-- ["Garage Club House"] = {
	    -- positionTo = { ['x'] = 1110.1051025391, ['y'] = -3165.2983398438, ['z'] = -38.518592834473, nom = "Sortir du Club House"},
		-- positionFrom = { ['x'] = 1950.2071533203, ['y'] = 3824.5368652344, ['z'] = 32.15149307251, nom = "Entrer au Club House"},
	-- },
-- }
	...
	-- while true do
		...
		-- for k, j in pairs(TeleportFromTo) do
			...
			-- if(Vdist(pos.x, pos.y, pos.z, j.positionFrom.x, j.positionFrom.y, j.positionFrom.z) < 150.0) then
				--DrawMarker(1, j.positionFrom.x, j.positionFrom.y, j.positionFrom.z - 0.99, 0, 0, 0, 0, 0, 0, 4.5, 4.5, 0.5001, 0, 255, 0, 105, 0, 0, 2, 2, 0, 0, 0)
				-- if(Vdist(pos.x, pos.y, pos.z, j.positionFrom.x, j.positionFrom.y, j.positionFrom.z) < 5.0)then
					-- if(Vdist(pos.x, pos.y, pos.z, j.positionFrom.x, j.positionFrom.y, j.positionFrom.z) < 3.0)then
						-- ClearPrints()
						--DisplayHelpText("~h~Appuyez sur ~INPUT_PICKUP~ ~r~pour ".. j.positionFrom.nom,6, 6, 6.0, 6.0,6.0, 255, 255, 255, 255)
			...
			-- if(Vdist(pos.x, pos.y, pos.z, j.positionTo.x, j.positionTo.y, j.positionTo.z) < 150.0) then
				--DrawMarker(1, j.positionTo.x, j.positionTo.y, j.positionTo.z - 0.99, 0, 0, 0, 0, 0, 0, 4.5, 4.5, 0.5001, 0, 255, 0, 105, 0, 0, 2, 2, 0, 0, 0)
				-- if(Vdist(pos.x, pos.y, pos.z, j.positionTo.x, j.positionTo.y, j.positionTo.z) < 5.0)then
					-- if(Vdist(pos.x, pos.y, pos.z, j.positionTo.x, j.positionTo.y, j.positionTo.z) < 1.0)then
						-- ClearPrints()
						--DisplayHelpText("~h~Appuyez sur ~INPUT_PICKUP~ ~b~pour ".. j.positionTo.nom,1, 1, 0.5, 0.8, 0.9, 255, 255, 255, 255)
						-- DrawSubtitleTimed(2000, 1)
						-- if IsControlJustPressed(1, 38) then
							-- DoScreenFadeOut(1000)
							-- Citizen.Wait(2000)
