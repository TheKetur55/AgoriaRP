...
Config.MaxWidth = 1.0 -- Will complete soon
Config.MaxHeight = 1.0
Config.MaxLength = 1.0
Config.FuelNeed = 10.0
    ...
    while true do 
        ...
        if Vehicle.Vehicle ~= nil then
                ...
                if IsVehicleSeatFree(Vehicle.Vehicle, -1) and GetVehicleEngineHealth(Vehicle.Vehicle) <= Config.DamageNeeded  then
                    ESX.Game.Utils.DrawText3D({x = Vehicle.Coords.x, y = Vehicle.Coords.y, z = Vehicle.Coords.z}, 'Pressez [~g~SHIFT~w~] et [~g~E~w~] pour pousser le véhicule', 0.0)
                end
                
                  if IsVehicleSeatFree(Vehicle.Vehicle, -1) and GetVehicleFuelLevel(Vehicle.Vehicle) <= Config.FuelNeed  then
                    ESX.Game.Utils.DrawText3D({x = Vehicle.Coords.x, y = Vehicle.Coords.y, z = Vehicle.Coords.z}, 'Pressez [~g~SHIFT~w~] et [~g~E~w~] pour pousser le véhicule', 0.0)
                end
