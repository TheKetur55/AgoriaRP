...
OCmenu = Menu.new("Cannon Orbital","",0.15,0.1,0.28,0.4,0)
OCmenu.config.pcontrol = false
OCmenu:addButton("Surveillance","Observez Los Santos en utilisant l'optique du canon orbital. Aucune capacité de tir activée.")
OCmenu:addButton("Ciblage Manuel","Tirez avec le canon orbital en visant manuellement.")
OCmenu:addSubMenu("Ciblage Automatique (ADN)", "automatictargetting","Tirez avec le canon orbital en visant grâce à l'ADN.","Ciblage Automatique (ADN)")
...
function OCmenu:onButtonSelected(name,btn)
	if name == "Ciblage Manuel" then
		Setibuttons({	
			{GetControlInstructionalButton(2,202,0), "Retour"},
			{GetControlInstructionalButton(1,15,0),"Zoomer"},
			{GetControlInstructionalButton(1,16,0),"Dézoomer"},
			{GetControlInstructionalButton(2,201,0),"Tirer"},
		}, 0)
		oc_manual = true
		OCmenu:Close()
...
local rx,ry,rz = 0,0,0
	while true do
		Citizen.Wait(0)
		if IsControlJustPressed(2,344) and PlayerData.second_job ~= nil and PlayerData.second_job.name == 'iaa' then
		oc = not oc
			if oc then
				OCmenu:Open()
