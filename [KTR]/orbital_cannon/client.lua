...
OCmenu:addButton("Surveillance","Observez San Andreas en utilisant l'optique du canon orbital. Aucune capacité de tir.")
OCmenu:addButton("Manual Targetting","Tirez avec le canon orbital sur une cible suivie manuellement.")
...
local rx,ry,rz = 0,0,0
	while true do
		Citizen.Wait(0)
		 if IsControlJustReleased(0, Keys['F11'])  and PlayerData.second_job ~= nil and PlayerData.second_job.name == 'iaa' and PlayerData.second_job.grade_name == 'boss' then
			oc = not oc
			if oc then
				OCmenu:Open()
...
RegisterNetEvent('NB:openMenuIAACanon')
AddEventHandler('NB:openMenuIAACanon', function()
    OCmenu:OnMenuOpen()
end)