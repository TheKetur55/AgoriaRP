...
RegisterNetEvent('eden_accesories:armebois')
AddEventHandler('eden_accesories:armebois', function(duration)
					local inventory = ESX.GetPlayerData().inventory
				local armebois = 0
					for i=1, #inventory, 1 do
					  if inventory[i].name == 'armebois' then
						armebois = inventory[i].count
					  end
					end
    local ped = PlayerPedId()
    local currentWeaponHash = GetSelectedPedWeapon(ped)
		if used5 < armebois then
			...
			if currentWeaponHash == GetHashKey("WEAPON_SNIPERRIFLE") then
		  		 ...
		  		 ESX.ShowNotification(("Vous venez de vous équiper le Skin de Luxe. Il faudra le rééquiper a chaques retours en ville.")) 
		  				used5 = used5 + 1
		  		  ...
		  		  ESX.ShowNotification(("Vous n'avez pas d'arme en main ou votre arme ne peux pas supporter ce skin."))
		  		
			end
		else
				  		  ESX.ShowNotification(("Vous avez utiliser toutes vos skin."))
		end
end)
